...
LLVM_CORE_ABI extern cl::opt<ChangePrinter> PrintChanged;

// Returns true if printing before/after some pass is enabled, whether all
// passes or a specific pass.
LLVM_CORE_ABI bool shouldPrintBeforeSomePass();
LLVM_CORE_ABI bool shouldPrintAfterSomePass();

// Returns true if we should print before/after a specific pass. The argument
// should be the pass ID, e.g. "instcombine".
LLVM_CORE_ABI bool shouldPrintBeforePass(StringRef PassID);
LLVM_CORE_ABI bool shouldPrintAfterPass(StringRef PassID);

// Returns true if we should print before/after all passes.
LLVM_CORE_ABI bool shouldPrintBeforeAll();
LLVM_CORE_ABI bool shouldPrintAfterAll();

// The list of passes to print before/after, if we only want to print
// before/after specific passes.
LLVM_CORE_ABI std::vector<std::string> printBeforePasses();
LLVM_CORE_ABI std::vector<std::string> printAfterPasses();

// Returns true if we should always print the entire module.
LLVM_CORE_ABI bool forcePrintModuleIR();

// Returns true if we should print the entire function for loop passes.
LLVM_CORE_ABI bool forcePrintFuncIR();

// Return true if -filter-passes is empty or contains the pass name.
LLVM_CORE_ABI bool isPassInPrintList(StringRef PassName);
LLVM_CORE_ABI bool isFilterPassesEmpty();

// Returns true if we should print the function.
LLVM_CORE_ABI bool isFunctionInPrintList(StringRef FunctionName);
...
LLVM_CORE_ABI std::error_code cleanUpTempFiles(ArrayRef<std::string> FileName);
...
// of the diff.
LLVM_CORE_ABI std::string doSystemDiff(StringRef Before, StringRef After,

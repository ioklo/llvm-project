...
class [[nodiscard]] ConstantFPRange {
  ...
  LLVM_CORE_ABI void makeEmpty();
  LLVM_CORE_ABI void makeFull();

  /// Initialize a full or empty set for the specified semantics.
  LLVM_CORE_ABI explicit ConstantFPRange(const fltSemantics &Sem,
                                         bool IsFullSet);

public:
  /// Initialize a range to hold the single specified value.
  LLVM_CORE_ABI explicit ConstantFPRange(const APFloat &Value);

  /// Initialize a range of values explicitly.
  /// Note: If \p LowerVal is greater than \p UpperVal, please use the canonical
  /// form [Inf, -Inf].
  LLVM_CORE_ABI ConstantFPRange(APFloat LowerVal, APFloat UpperVal,
  ...
  LLVM_CORE_ABI static ConstantFPRange getFinite(const fltSemantics &Sem);

  /// Helper for [-inf, inf] to represent all non-NaN values.
  LLVM_CORE_ABI static ConstantFPRange getNonNaN(const fltSemantics &Sem);
  ...
  LLVM_CORE_ABI static ConstantFPRange
  getNaNOnly(const fltSemantics &Sem, bool MayBeQNaN, bool MayBeSNaN);
  ...
  /// std::nullopt.
  LLVM_CORE_ABI static std::optional<ConstantFPRange>
  ...
  LLVM_CORE_ABI bool fcmp(FCmpInst::Predicate Pred,
  ...
  LLVM_CORE_ABI bool isNaNOnly() const;
  ...
  LLVM_CORE_ABI bool contains(const APFloat &Val) const;

  /// Return true if the other range is a subset of this one.
  LLVM_CORE_ABI bool contains(const ConstantFPRange &CR) const;

  /// If this set contains a single element, return it, otherwise return null.
  /// If \p ExcludesNaN is true, return the non-NaN single element.
  LLVM_CORE_ABI const APFloat *getSingleElement(bool ExcludesNaN = false) const;
  ...
  LLVM_CORE_ABI std::optional<bool> getSignBit() const;

  /// Return true if this range is equal to another range.
  LLVM_CORE_ABI bool operator==(const ConstantFPRange &CR) const;
  ...
  LLVM_CORE_ABI FPClassTest classify() const;
  ...
  /// another range.
  LLVM_CORE_ABI ConstantFPRange intersectWith(const ConstantFPRange &CR) const;
  ...
  LLVM_CORE_ABI ConstantFPRange unionWith(const ConstantFPRange &CR) const;
};

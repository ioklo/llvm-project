...
public:
  ...
  LLVM_CORE_ABI MDString *createString(StringRef Str);

  /// Return the given constant as metadata.
  LLVM_CORE_ABI ConstantAsMetadata *createConstant(Constant *C);
  ...
  /// setting.
  LLVM_CORE_ABI MDNode *createFPMath(float Accuracy);
  ...
  LLVM_CORE_ABI MDNode *createBranchWeights(uint32_t TrueWeight,
                                            uint32_t FalseWeight,
                                            bool IsExpected = false);

  /// Return metadata containing two branch weights, with significant bias
  /// towards `true` destination.
  LLVM_CORE_ABI MDNode *createLikelyBranchWeights();

  /// Return metadata containing two branch weights, with significant bias
  /// towards `false` destination.
  LLVM_CORE_ABI MDNode *createUnlikelyBranchWeights();

  /// Return metadata containing a number of branch weights.
  /// @param Weights the weights of all the branches
  /// @param Do these weights come from __builtin_expect*
  LLVM_CORE_ABI MDNode *createBranchWeights(ArrayRef<uint32_t> Weights,
                                            bool IsExpected = false);

  /// Return metadata specifying that a branch or switch is unpredictable.
  LLVM_CORE_ABI MDNode *createUnpredictable();
  ...
  LLVM_CORE_ABI MDNode *
  createFunctionEntryCount(uint64_t Count, bool Synthetic,
                           const DenseSet<GlobalValue::GUID> *Imports);

  /// Return metadata containing the section prefix for a function.
  LLVM_CORE_ABI MDNode *createFunctionSectionPrefix(StringRef Prefix);

  /// Return metadata containing the pseudo probe descriptor for a function.
  LLVM_CORE_ABI MDNode *createPseudoProbeDesc(uint64_t GUID, uint64_t Hash,
                                              StringRef FName);
  ...
  LLVM_CORE_ABI MDNode *createRange(const APInt &Lo, const APInt &Hi);

  /// Return metadata describing the range [Lo, Hi).
  LLVM_CORE_ABI MDNode *createRange(Constant *Lo, Constant *Hi);
  ...
  /// calls.
  LLVM_CORE_ABI MDNode *createCallees(ArrayRef<Function *> Callees);
  ...
  LLVM_CORE_ABI MDNode *createPCSections(ArrayRef<PCSection> Sections);
...
protected:
  ...
  LLVM_CORE_ABI MDNode *createAnonymousAARoot(StringRef Name = StringRef(),
...
public:
  ...
  /// name.
  LLVM_CORE_ABI MDNode *createTBAARoot(StringRef Name);

  /// Return metadata appropriate for an alias scope domain node with
  /// the given name. This may be identified (uniqued) with other roots with
  /// the same name.
  LLVM_CORE_ABI MDNode *createAliasScopeDomain(StringRef Name);
  ...
  LLVM_CORE_ABI MDNode *createAliasScope(StringRef Name, MDNode *Domain);

  /// Return metadata for a non-root TBAA node with the given name,
  /// parent in the TBAA tree, and value for 'pointsToConstantMemory'.
  LLVM_CORE_ABI MDNode *createTBAANode(StringRef Name, MDNode *Parent,
  ...
  LLVM_CORE_ABI MDNode *createTBAAStructNode(ArrayRef<TBAAStructField> Fields);
  ...
  LLVM_CORE_ABI MDNode *createTBAAScalarTypeNode(StringRef Name, MDNode *Parent,
                                                 uint64_t Offset = 0);

  /// Return metadata for a TBAA tag node with the given
  /// base type, access type and offset relative to the base type.
  LLVM_CORE_ABI MDNode *createTBAAStructTagNode(MDNode *BaseType,
                                                MDNode *AccessType,
                                                uint64_t Offset,
                                                bool IsConstant = false);
  ...
  LLVM_CORE_ABI MDNode *createTBAATypeNode(
  ...
  LLVM_CORE_ABI MDNode *createTBAAAccessTag(MDNode *BaseType,
  ...
  /// access tag.
  LLVM_CORE_ABI MDNode *createMutableTBAAAccessTag(MDNode *Tag);

  /// Return metadata containing an irreducible loop header weight.
  LLVM_CORE_ABI MDNode *createIrrLoopHeaderWeight(uint64_t Weight);
};

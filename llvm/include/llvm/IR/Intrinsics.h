...
LLVM_CORE_ABI std::string getName(ID Id, ArrayRef<Type *> Tys, Module *M,
...
LLVM_CORE_ABI std::string getNameNoUnnamedTypes(ID Id, ArrayRef<Type *> Tys);

/// Return the function type for an intrinsic.
FunctionType *getType(LLVMContext &Context, ID id, ArrayRef<Type *> Tys = {});

/// Returns true if the intrinsic can be overloaded.
LLVM_CORE_ABI bool isOverloaded(ID id);

/// isTargetIntrinsic - Returns true if IID is an intrinsic specific to a
/// certain target. If it is a generic intrinsic false is returned.
LLVM_CORE_ABI bool isTargetIntrinsic(ID IID);

LLVM_CORE_ABI ID lookupIntrinsicID(StringRef Name);

/// Return the attributes for an intrinsic.
LLVM_CORE_ABI AttributeList getAttributes(LLVMContext &C, ID id);
...
/// the intrinsic.
LLVM_CORE_ABI Function *getOrInsertDeclaration(Module *M, ID id,
                                               ArrayRef<Type *> Tys = {});

LLVM_DEPRECATED("Use getOrInsertDeclaration instead", "getOrInsertDeclaration")
...
LLVM_CORE_ABI Function *getDeclarationIfExists(const Module *M, ID id);

/// This version supports overloaded intrinsics.
LLVM_CORE_ABI Function *getDeclarationIfExists(Module *M, ID id,
                                               ArrayRef<Type *> Tys,
                                               FunctionType *FT = nullptr);

/// Map a Clang builtin name to an intrinsic ID.
LLVM_CORE_ABI ID getIntrinsicForClangBuiltin(StringRef TargetPrefix,
                                             StringRef BuiltinName);

/// Map a MS builtin name to an intrinsic ID.
LLVM_CORE_ABI ID getIntrinsicForMSBuiltin(StringRef TargetPrefix,
                                          StringRef BuiltinName);

/// Returns true if the intrinsic ID is for one of the "Constrained
/// Floating-Point Intrinsics".
LLVM_CORE_ABI bool isConstrainedFPIntrinsic(ID QID);

/// Returns true if the intrinsic ID is for one of the "Constrained
/// Floating-Point Intrinsics" that take rounding mode metadata.
LLVM_CORE_ABI bool hasConstrainedFPRoundingModeOperand(ID QID);
...
/// otherwise.
LLVM_CORE_ABI MatchIntrinsicTypesResult
...
LLVM_CORE_ABI bool matchIntrinsicVarArg(bool isVarArg,
...
LLVM_CORE_ABI bool getIntrinsicSignature(Intrinsic::ID, FunctionType *FT,
                                         SmallVectorImpl<Type *> &ArgTys);

/// Same as previous, but accepts a Function instead of ID and FunctionType.
LLVM_CORE_ABI bool getIntrinsicSignature(Function *F,
                                         SmallVectorImpl<Type *> &ArgTys);
...
LLVM_CORE_ABI std::optional<Function *> remangleIntrinsicFunction(Function *F);

...
public:
  ...
  LLVM_CORE_ABI DbgRecordParamRef(const T *Param);
  ...
  /// IR.
  LLVM_CORE_ABI explicit DbgRecordParamRef(const MDNode *Param);

  /// Get the underlying type.
  ///
  /// \pre !*this or \c isa<T>(getAsMDNode()).
  /// @{
  LLVM_CORE_ABI T *get() const;
...
public:
  ...
  ///@{
  LLVM_CORE_ABI void deleteRecord();
  LLVM_CORE_ABI DbgRecord *clone() const;
  ...
  LLVM_CORE_ABI bool isIdenticalToWhenDefined(const DbgRecord &R) const;
  ...
  LLVM_CORE_ABI bool isEquivalentTo(const DbgRecord &R) const;
  ...
  LLVM_CORE_ABI BasicBlock *getBlock();
  LLVM_CORE_ABI const BasicBlock *getBlock() const;

  LLVM_CORE_ABI Function *getFunction();
  LLVM_CORE_ABI const Function *getFunction() const;

  LLVM_CORE_ABI Module *getModule();
  LLVM_CORE_ABI const Module *getModule() const;

  LLVM_CORE_ABI LLVMContext &getContext();
  LLVM_CORE_ABI const LLVMContext &getContext() const;

  LLVM_CORE_ABI const Instruction *getInstruction() const;
  LLVM_CORE_ABI const BasicBlock *getParent() const;
  LLVM_CORE_ABI BasicBlock *getParent();
  ...
  LLVM_CORE_ABI void insertBefore(DbgRecord *InsertBefore);
  LLVM_CORE_ABI void insertAfter(DbgRecord *InsertAfter);
  LLVM_CORE_ABI void moveBefore(DbgRecord *MoveBefore);
  LLVM_CORE_ABI void moveAfter(DbgRecord *MoveAfter);

  LLVM_CORE_ABI void insertBefore(self_iterator InsertBefore);
  LLVM_CORE_ABI void insertAfter(self_iterator InsertAfter);
  LLVM_CORE_ABI void moveBefore(self_iterator MoveBefore);
  LLVM_CORE_ABI void moveAfter(self_iterator MoveAfter);
...
class DbgLabelRecord : public DbgRecord {
  ...
  /// parsing only.
  LLVM_CORE_ABI DbgLabelRecord(MDNode *Label, MDNode *DL);

public:
  LLVM_CORE_ABI DbgLabelRecord(DILabel *Label, DebugLoc DL);
  ...
  /// crash.
  LLVM_CORE_ABI static DbgLabelRecord *
  createUnresolvedDbgLabelRecord(MDNode *Label, MDNode *DL);

  LLVM_CORE_ABI DbgLabelRecord *clone() const;
  LLVM_CORE_ABI void print(raw_ostream &O, bool IsForDebug = false) const;
  LLVM_CORE_ABI void print(raw_ostream &ROS, ModuleSlotTracker &MST,
                           bool IsForDebug) const;
  LLVM_CORE_ABI DbgLabelInst *
...
public:
  ...
  LLVM_CORE_ABI DbgVariableRecord(const DbgVariableIntrinsic *DVI);
  LLVM_CORE_ABI DbgVariableRecord(const DbgVariableRecord &DVR);
  ...
  LLVM_CORE_ABI DbgVariableRecord(Metadata *Location, DILocalVariable *DV,
                                  DIExpression *Expr, const DILocation *DI,
                                  LocationType Type = LocationType::Value);
  LLVM_CORE_ABI DbgVariableRecord(Metadata *Value, DILocalVariable *Variable,
...
private:
  ...
  LLVM_CORE_ABI DbgVariableRecord(LocationType Type, Metadata *Val,
...
public:
  ...
  LLVM_CORE_ABI static DbgVariableRecord *
  ...
  LLVM_CORE_ABI iterator_range<location_op_iterator> location_ops() const;

  LLVM_CORE_ABI Value *getVariableLocationOp(unsigned OpIdx) const;

  LLVM_CORE_ABI void replaceVariableLocationOp(Value *OldValue, Value *NewValue,
                                               bool AllowEmpty = false);
  LLVM_CORE_ABI void replaceVariableLocationOp(unsigned OpIdx, Value *NewValue);
  /// Adding a new location operand will always result in this intrinsic using
  /// an ArgList, and must always be accompanied by a new expression that uses
  /// the new operand.
  LLVM_CORE_ABI void addVariableLocationOps(ArrayRef<Value *> NewValues,
                                            DIExpression *NewExpr);

  LLVM_CORE_ABI unsigned getNumVariableLocationOps() const;
  ...
  LLVM_CORE_ABI void setKillLocation();
  LLVM_CORE_ABI bool isKillLocation() const;
  ...
  LLVM_CORE_ABI std::optional<DbgVariableFragmentInfo> getFragment() const;
  ...
  /// is described.
  LLVM_CORE_ABI std::optional<uint64_t> getFragmentSizeInBits() const;
  ...
  LLVM_CORE_ABI void setAssignId(DIAssignID *New);
  void setAddress(Value *V) { resetDebugValue(1, ValueAsMetadata::get(V)); }
  /// Kill the address component.
  LLVM_CORE_ABI void setKillAddress();
  ...
  LLVM_CORE_ABI bool isKillAddress() const;

  /// @}

  LLVM_CORE_ABI DbgVariableRecord *clone() const;
  ...
  LLVM_CORE_ABI DbgVariableIntrinsic *
  createDebugIntrinsic(Module *M, Instruction *InsertBefore) const;
...
public:
  ...
  LLVM_CORE_ABI void removeMarker();
  LLVM_CORE_ABI void dump() const;
  ...
  getDbgRecordRange();
  LLVM_CORE_ABI iterator_range<simple_ilist<DbgRecord>::const_iterator>
  ...
  /// afterwards.
  LLVM_CORE_ABI void absorbDebugValues(DbgMarker &Src, bool InsertAtHead);
  ...
  LLVM_CORE_ABI void insertDbgRecord(DbgRecord *New, bool InsertAtHead);
  /// Insert a DbgRecord prior to a DbgRecord contained within this marker.
  LLVM_CORE_ABI void insertDbgRecord(DbgRecord *New, DbgRecord *InsertBefore);
  /// Insert a DbgRecord after a DbgRecord contained within this marker.
  LLVM_CORE_ABI void insertDbgRecordAfter(DbgRecord *New,
  ...
  LLVM_CORE_ABI void dropOneDbgRecord(DbgRecord *DR);
  ...
  /// for that.
  LLVM_CORE_ABI static DbgMarker EmptyDbgMarker;

...
struct VFShape {
  ...
  LLVM_CORE_ABI bool hasValidParameterList() const;
};
...
/// or return values.
LLVM_CORE_ABI std::optional<VFInfo>
tryDemangleForVFABI(StringRef MangledName, const FunctionType *FTy);

/// Retrieve the `VFParamKind` from a string token.
LLVM_CORE_ABI VFParamKind getVFParamKindFromString(const StringRef Token);
...
LLVM_CORE_ABI FunctionType *createFunctionType(const VFInfo &Info,
                                               const FunctionType *ScalarFTy);

/// Overwrite the Vector Function ABI variants attribute with the names provide
/// in \p VariantMappings.
LLVM_CORE_ABI void setVectorVariantNames(CallInst *CI,

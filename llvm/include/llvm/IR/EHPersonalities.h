...
/// Unknown.
LLVM_CORE_ABI EHPersonality classifyEHPersonality(const Value *Pers);

LLVM_CORE_ABI StringRef getEHPersonalityName(EHPersonality Pers);

LLVM_CORE_ABI EHPersonality getDefaultEHPersonality(const Triple &T);
...
LLVM_CORE_ABI bool canSimplifyInvokeNoUnwind(const Function *F);
...
/// expensive.
LLVM_CORE_ABI DenseMap<BasicBlock *, ColorVector> colorEHFunclets(Function &F);

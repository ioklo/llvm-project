...
protected:
  ...
  LLVM_CORE_ABI void *operator new(size_t Size, HungOffOperandsAllocMarker);

  /// Allocate a User with the operands co-allocated.
  ///
  /// This is used for subclasses which have a fixed number of operands.
  LLVM_CORE_ABI void *operator new(size_t Size,
  ...
  LLVM_CORE_ABI void allocHungoffUses(unsigned N, bool IsPhi = false);

  /// Grow the number of hung off uses.  Note that allocHungoffUses
  /// should be called if there are no uses.
  LLVM_CORE_ABI void growHungoffUses(unsigned N, bool IsPhi = false);
...
public:
  ...
  LLVM_CORE_ABI void operator delete(void *Usr);
...
public:
  ...
  LLVM_CORE_ABI ArrayRef<const uint8_t> getDescriptor() const;

  /// Returns the descriptor co-allocated with this User instance.
  LLVM_CORE_ABI MutableArrayRef<uint8_t> getDescriptor();
  ...
  /// from happening.
  LLVM_CORE_ABI bool isDroppable() const;
  ...
  LLVM_CORE_ABI bool replaceUsesOfWith(Value *From, Value *To);

...
public:
  ...
  LLVM_CORE_ABI DbgMarker *createMarker(Instruction *I);
  LLVM_CORE_ABI DbgMarker *createMarker(InstListType::iterator It);
  ...
  /// if necessary.
  LLVM_CORE_ABI void setIsNewDbgInfoFormat(bool NewFlag);
  LLVM_CORE_ABI void setNewDbgInfoFormatFlag(bool NewFlag);
  ...
  LLVM_CORE_ABI void setTrailingDbgRecords(DbgMarker *M);
  ...
  LLVM_CORE_ABI void deleteTrailingDbgRecords();

  LLVM_CORE_ABI void dumpDbgValues() const;
  ...
  LLVM_CORE_ABI DbgMarker *getMarker(InstListType::iterator It);
  ...
  LLVM_CORE_ABI DbgMarker *getNextMarker(Instruction *I);

  /// Insert a DbgRecord into a block at the position given by \p I.
  LLVM_CORE_ABI void insertDbgRecordAfter(DbgRecord *DR, Instruction *I);

  /// Insert a DbgRecord into a block at the position given by \p Here.
  LLVM_CORE_ABI void insertDbgRecordBefore(DbgRecord *DR,
  ...
  LLVM_CORE_ABI void flushTerminatorDbgRecords();
...
private:
  LLVM_CORE_ABI void setParent(Function *parent);
  ...
  LLVM_CORE_ABI explicit BasicBlock(LLVMContext &C, const Twine &Name = "",
...
public:
  ...
  friend LLVM_CORE_ABI void Instruction::removeFromParent();
  friend LLVM_CORE_ABI BasicBlock::iterator Instruction::eraseFromParent();
  friend LLVM_CORE_ABI BasicBlock::iterator
  Instruction::insertInto(BasicBlock *BB, BasicBlock::iterator It);
  ...
  friend LLVM_CORE_ABI void
  Instruction::insertBefore(BasicBlock::iterator InsertPos);
  friend LLVM_CORE_ABI void Instruction::insertAfter(Instruction *InsertPos);
  friend LLVM_CORE_ABI void
  Instruction::insertAfter(BasicBlock::iterator InsertPos);
  friend LLVM_CORE_ABI void
  Instruction::insertBefore(BasicBlock &BB, InstListType::iterator InsertPos);
  friend LLVM_CORE_ABI void
  Instruction::moveBeforeImpl(BasicBlock &BB, InstListType::iterator I,
                              bool Preserve);
  friend LLVM_CORE_ABI iterator_range<DbgRecord::self_iterator>
  ...
  LLVM_CORE_ABI const CallInst *getTerminatingDeoptimizeCall() const;
  ...
  LLVM_CORE_ABI const CallInst *getPostdominatingDeoptimizeCall() const;
  ...
  LLVM_CORE_ABI const CallInst *getTerminatingMustTailCall() const;
  ...
  LLVM_CORE_ABI const Instruction *getFirstNonPHI() const;
  LLVM_DEPRECATED("Use iterators as instruction positions instead",
                  "getFirstNonPHIIt")
  LLVM_CORE_ABI Instruction *getFirstNonPHI();
  ...
  LLVM_CORE_ABI InstListType::const_iterator getFirstNonPHIIt() const;
  ...
  /// is true.
  LLVM_CORE_ABI InstListType::const_iterator
  ...
  LLVM_CORE_ABI const_iterator getFirstInsertionPt() const;
  ...
  LLVM_CORE_ABI const_iterator getFirstNonPHIOrDbgOrAlloca() const;
  ...
  LLVM_CORE_ABI const Instruction *getFirstMayFaultInst() const;
  ...
  LLVM_CORE_ABI SymbolTableList<BasicBlock>::iterator eraseFromParent();
  ...
  LLVM_CORE_ABI void moveBefore(SymbolTableList<BasicBlock>::iterator MovePos);

  /// Unlink this basic block from its current function and insert it
  /// right after \p MovePos in the function \p MovePos lives in.
  LLVM_CORE_ABI void moveAfter(BasicBlock *MovePos);
  ...
  LLVM_CORE_ABI void insertInto(Function *Parent,
                                BasicBlock *InsertBefore = nullptr);

  /// Return the predecessor of this block if it has a single predecessor
  /// block. Otherwise return a null pointer.
  LLVM_CORE_ABI const BasicBlock *getSinglePredecessor() const;
  ...
  LLVM_CORE_ABI const BasicBlock *getUniquePredecessor() const;
  ...
  LLVM_CORE_ABI bool hasNPredecessors(unsigned N) const;

  /// Return true if this block has N predecessors or more.
  LLVM_CORE_ABI bool hasNPredecessorsOrMore(unsigned N) const;

  /// Return the successor of this block if it has a single successor.
  /// Otherwise return a null pointer.
  ///
  /// This method is analogous to getSinglePredecessor above.
  LLVM_CORE_ABI const BasicBlock *getSingleSuccessor() const;
  ...
  LLVM_CORE_ABI const BasicBlock *getUniqueSuccessor() const;
  ...
  LLVM_CORE_ABI iterator_range<phi_iterator> phis();
...
private:
  ...
  LLVM_CORE_ABI void spliceDebugInfoEmptyBlock(BasicBlock::iterator ToIt,
                                               BasicBlock *FromBB,
  ...
  /// to another.
  LLVM_CORE_ABI void spliceDebugInfo(BasicBlock::iterator ToIt,
                                     BasicBlock *FromBB,
                                     BasicBlock::iterator FromBeginIt,
                                     BasicBlock::iterator FromEndIt);
  LLVM_CORE_ABI void spliceDebugInfoImpl(BasicBlock::iterator ToIt,
                                         BasicBlock *FromBB,
                                         BasicBlock::iterator FromBeginIt,
                                         BasicBlock::iterator FromEndIt);
...
public:
  ...
  LLVM_CORE_ABI ValueSymbolTable *getValueSymbolTable();
  ...
  /// values the same.
  LLVM_CORE_ABI void removePredecessor(BasicBlock *Pred,
                                       bool KeepOneInputPHIs = false);

  LLVM_CORE_ABI bool canSplitPredecessors() const;
  ...
  LLVM_CORE_ABI BasicBlock *
  splitBasicBlock(iterator I, const Twine &BBName = "", bool Before = false);
  ...
  /// function.
  LLVM_CORE_ABI BasicBlock *splitBasicBlockBefore(iterator I,
                                                  const Twine &BBName = "");
  ...
  LLVM_CORE_ABI void splice(BasicBlock::iterator ToIt, BasicBlock *FromBB,
                            BasicBlock::iterator FromBeginIt,
                            BasicBlock::iterator FromEndIt);

  /// Erases a range of instructions from \p FromIt to (not including) \p ToIt.
  /// \Returns \p ToIt.
  LLVM_CORE_ABI BasicBlock::iterator erase(BasicBlock::iterator FromIt,
  ...
  LLVM_CORE_ABI void replacePhiUsesWith(BasicBlock *Old, BasicBlock *New);

  /// Update all phi nodes in this basic block's successors to refer to basic
  /// block \p New instead of basic block \p Old.
  LLVM_CORE_ABI void replaceSuccessorsPhiUsesWith(BasicBlock *Old,
                                                  BasicBlock *New);

  /// Update all phi nodes in this basic block's successors to refer to basic
  /// block \p New instead of to it.
  LLVM_CORE_ABI void replaceSuccessorsPhiUsesWith(BasicBlock *New);
  ...
  LLVM_CORE_ABI bool isLandingPad() const;

  /// Return the landingpad instruction associated with the landing pad.
  LLVM_CORE_ABI const LandingPadInst *getLandingPadInst() const;
  ...
  LLVM_CORE_ABI bool isLegalToHoistInto() const;

  /// Return true if this is the entry block of the containing function.
  /// This method can only be used on blocks that have a parent function.
  LLVM_CORE_ABI bool isEntryBlock() const;

  LLVM_CORE_ABI std::optional<uint64_t> getIrrLoopHeaderWeight() const;
  ...
  LLVM_CORE_ABI void renumberInstructions();
  ...
  LLVM_CORE_ABI void validateInstrOrdering() const;
...
LLVM_CORE_ABI BasicBlock::iterator skipDebugIntrinsics(BasicBlock::iterator It);

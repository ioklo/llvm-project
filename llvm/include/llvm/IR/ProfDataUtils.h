...
LLVM_CORE_ABI bool hasProfMD(const Instruction &I);

/// Checks if an MDNode contains Branch Weight Metadata
LLVM_CORE_ABI bool isBranchWeightMD(const MDNode *ProfileData);
...
/// otherwise.
LLVM_CORE_ABI bool hasBranchWeightMD(const Instruction &I);
...
LLVM_CORE_ABI bool hasValidBranchWeightMD(const Instruction &I);
...
/// Nullptr otherwise.
LLVM_CORE_ABI MDNode *getBranchWeightMDNode(const Instruction &I);
...
/// Nullptr otherwise.
LLVM_CORE_ABI MDNode *getValidBranchWeightMDNode(const Instruction &I);

/// Check if Branch Weight Metadata has an "expected" field from an llvm.expect*
/// intrinsic
LLVM_CORE_ABI bool hasBranchWeightOrigin(const Instruction &I);

/// Check if Branch Weight Metadata has an "expected" field from an llvm.expect*
/// intrinsic
LLVM_CORE_ABI bool hasBranchWeightOrigin(const MDNode *ProfileData);

/// Return the offset to the first branch weight data
LLVM_CORE_ABI unsigned getBranchWeightOffset(const MDNode *ProfileData);

LLVM_CORE_ABI unsigned getNumBranchWeights(const MDNode &ProfileData);
...
/// will be cleared.
LLVM_CORE_ABI bool extractBranchWeights(const MDNode *ProfileData,
...
LLVM_CORE_ABI void
extractFromBranchWeightMD32(const MDNode *ProfileData,
                            SmallVectorImpl<uint32_t> &Weights);

/// Faster version of extractBranchWeights() that skips checks and must only
/// be called with "branch_weights" metadata nodes. Supports uint64_t.
LLVM_CORE_ABI void
extractFromBranchWeightMD64(const MDNode *ProfileData,
...
/// \returns True if weights were extracted, False otherwise. When false Weights
/// will be cleared.
LLVM_CORE_ABI bool extractBranchWeights(const Instruction &I,
                                        SmallVectorImpl<uint32_t> &Weights);
...
LLVM_CORE_ABI bool extractBranchWeights(const Instruction &I, uint64_t &TrueVal,
                                        uint64_t &FalseVal);
...
/// metadata was found.
LLVM_CORE_ABI bool extractProfTotalWeight(const MDNode *ProfileData,
...
/// \returns True on success with profile total weights filled in. False if no
/// metadata was found.
LLVM_CORE_ABI bool extractProfTotalWeight(const Instruction &I,
                                          uint64_t &TotalWeights);
...
LLVM_CORE_ABI void setBranchWeights(Instruction &I, ArrayRef<uint32_t> Weights,
                                    bool IsExpected);

/// Scaling the profile data attached to 'I' using the ratio of S/T.
LLVM_CORE_ABI void scaleProfData(Instruction &I, uint64_t S, uint64_t T);

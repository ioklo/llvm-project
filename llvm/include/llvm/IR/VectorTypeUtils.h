...
LLVM_CORE_ABI Type *toVectorizedStructTy(StructType *StructTy, ElementCount EC);

/// A helper for converting structs of vector types to structs of scalar types.
/// Note: Only unpacked literal struct types are supported.
LLVM_CORE_ABI Type *toScalarizedStructTy(StructType *StructTy);
...
LLVM_CORE_ABI bool isVectorizedStructTy(StructType *StructTy);

/// Returns true if `StructTy` is an unpacked literal struct where all elements
/// are scalars that can be used as vector element types.
LLVM_CORE_ABI bool canVectorizeStructTy(StructType *StructTy);

...
LLVM_CORE_ABI iterator_range<simple_ilist<DbgRecord>::iterator>
...
public:
  ...
  LLVM_DEPRECATED("Use BasicBlock::iterators for insertion instead",
                  "BasicBlock::iterator")
  LLVM_CORE_ABI InsertPosition(Instruction *InsertBefore);
  LLVM_CORE_ABI InsertPosition(BasicBlock *InsertAtEnd);
...
public:
  ...
  LLVM_CORE_ABI std::optional<simple_ilist<DbgRecord>::iterator>
  getDbgReinsertionPosition();

  /// Returns true if any DbgRecords are attached to this instruction.
  LLVM_CORE_ABI bool hasDbgRecords() const;
  ...
  LLVM_CORE_ABI void dropDbgRecords();

  /// Erase a single DbgRecord \p I that is attached to this instruction.
  LLVM_CORE_ABI void dropOneDbgRecord(DbgRecord *I);
  ...
  LLVM_CORE_ABI void handleMarkerRemoval();
...
protected:
  LLVM_CORE_ABI ~Instruction(); // Use deleteValue() to delete a generic Instruction.
...
public:
  ...
  LLVM_CORE_ABI const Module *getModule() const;
  Module *getModule() {
  ...
  LLVM_CORE_ABI const Function *getFunction() const;
  ...
  LLVM_CORE_ABI InstListType::iterator eraseFromParent();
  ...
  LLVM_CORE_ABI void insertBefore(Instruction *InsertPos);
  ...
  LLVM_CORE_ABI void insertAfter(Instruction *InsertPos);

  /// Insert an unlinked instruction into a basic block immediately after the
  /// specified position.
  LLVM_CORE_ABI void insertAfter(InstListType::iterator InsertPos);

  /// Inserts an unlinked instruction into \p ParentBB at position \p It and
  /// returns the iterator of the inserted instruction.
  LLVM_CORE_ABI InstListType::iterator insertInto(BasicBlock *ParentBB,
                                                  InstListType::iterator It);

  LLVM_CORE_ABI void insertBefore(BasicBlock &BB,
  ...
  /// getIterator to fetch the instruction iterator.
  LLVM_DEPRECATED("Use iterators as instruction positions", "")
  LLVM_CORE_ABI void moveBefore(Instruction *MovePos);

  /// Unlink this instruction from its current basic block and insert it into
  /// the basic block that MovePos lives in, right before MovePos.
  LLVM_CORE_ABI void moveBefore(InstListType::iterator InsertPos);
  ...
  LLVM_CORE_ABI void moveBeforePreserving(InstListType::iterator MovePos);

  /// Perform a \ref moveBefore operation, while signalling that the caller
  /// intends to preserve the original ordering of instructions. This implicitly
  /// means that any adjacent debug-info should move with this instruction.
  LLVM_CORE_ABI void moveBeforePreserving(BasicBlock &BB,
  ...
  LLVM_DEPRECATED("Use iterators as instruction positions", "")
  LLVM_CORE_ABI void moveBeforePreserving(Instruction *MovePos);

private:
  /// RemoveDIs project: all other moves implemented with this method,
  /// centralising debug-info updates into one place.
  LLVM_CORE_ABI void moveBeforeImpl(BasicBlock &BB, InstListType::iterator I,
                                    bool Preserve);
...
public:
  ...
  LLVM_CORE_ABI void moveBefore(BasicBlock &BB, InstListType::iterator I);

  /// Unlink this instruction from its current basic block and insert it into
  /// the basic block that MovePos lives in, right after MovePos.
  LLVM_CORE_ABI void moveAfter(Instruction *MovePos);

  /// Unlink this instruction from its current basic block and insert it into
  /// the basic block that MovePos lives in, right after MovePos.
  LLVM_CORE_ABI void moveAfter(InstListType::iterator MovePos);

  /// See \ref moveBeforePreserving .
  LLVM_CORE_ABI void moveAfterPreserving(Instruction *MovePos);
  ...
  LLVM_CORE_ABI bool comesBefore(const Instruction *Other) const;
  ...
  LLVM_CORE_ABI std::optional<InstListType::iterator>
  ...
  /// its operands.
  LLVM_CORE_ABI bool isOnlyUserOfAnyOperand();

  LLVM_CORE_ABI static const char *getOpcodeName(unsigned Opcode);
  ...
  // a debug location
  LLVM_CORE_ABI bool hasNonDebugLocLoopMetadata() const;
  ...
  LLVM_CORE_ABI void copyMetadata(const Instruction &SrcInst,
  ...
  /// metadata.
  LLVM_CORE_ABI void swapProfMetadata();
  ...
  LLVM_CORE_ABI void addAnnotationMetadata(StringRef Annotation);
  ...
  /// the existing node.
  LLVM_CORE_ABI void addAnnotationMetadata(SmallVector<StringRef> Annotations);
  /// Returns the AA metadata for this instruction.
  LLVM_CORE_ABI AAMDNodes getAAMetadata() const;

  /// Sets the AA metadata on this instruction from the AAMDNodes structure.
  LLVM_CORE_ABI void setAAMetadata(const AAMDNodes &N);
  ...
  LLVM_CORE_ABI bool extractProfTotalWeight(uint64_t &TotalVal) const;
  ...
  LLVM_CORE_ABI const DebugLoc &getStableDebugLoc() const;

  /// Set or clear the nuw flag on this instruction, which must be an operator
  /// which supports this flag. See LangRef.html for the meaning of this flag.
  LLVM_CORE_ABI void setHasNoUnsignedWrap(bool b = true);

  /// Set or clear the nsw flag on this instruction, which must be an operator
  /// which supports this flag. See LangRef.html for the meaning of this flag.
  LLVM_CORE_ABI void setHasNoSignedWrap(bool b = true);

  /// Set or clear the exact flag on this instruction, which must be an operator
  /// which supports this flag. See LangRef.html for the meaning of this flag.
  LLVM_CORE_ABI void setIsExact(bool b = true);

  /// Set or clear the nneg flag on this instruction, which must be a zext
  /// instruction.
  LLVM_CORE_ABI void setNonNeg(bool b = true);

  /// Determine whether the no unsigned wrap flag is set.
  LLVM_CORE_ABI bool hasNoUnsignedWrap() const LLVM_READONLY;

  /// Determine whether the no signed wrap flag is set.
  LLVM_CORE_ABI bool hasNoSignedWrap() const LLVM_READONLY;

  /// Determine whether the the nneg flag is set.
  LLVM_CORE_ABI bool hasNonNeg() const LLVM_READONLY;

  /// Return true if this operator has flags which may cause this instruction
  /// to evaluate to poison despite having non-poison inputs.
  LLVM_CORE_ABI bool hasPoisonGeneratingFlags() const LLVM_READONLY;

  /// Drops flags that may cause this instruction to evaluate to poison despite
  /// having non-poison inputs.
  LLVM_CORE_ABI void dropPoisonGeneratingFlags();

  /// Return true if this instruction has poison-generating metadata.
  LLVM_CORE_ABI bool hasPoisonGeneratingMetadata() const LLVM_READONLY;

  /// Drops metadata that may generate poison.
  LLVM_CORE_ABI void dropPoisonGeneratingMetadata();

  /// Return true if this instruction has poison-generating attribute.
  LLVM_CORE_ABI bool hasPoisonGeneratingReturnAttributes() const LLVM_READONLY;

  /// Drops return attributes that may generate poison.
  LLVM_CORE_ABI void dropPoisonGeneratingReturnAttributes();
  ...
  LLVM_CORE_ABI void dropUBImplyingAttrsAndMetadata();

  /// Determine whether the exact flag is set.
  LLVM_CORE_ABI bool isExact() const LLVM_READONLY;

  /// Set or clear all fast-math-flags on this instruction, which must be an
  /// operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setFast(bool B);

  /// Set or clear the reassociation flag on this instruction, which must be
  /// an operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasAllowReassoc(bool B);

  /// Set or clear the no-nans flag on this instruction, which must be an
  /// operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasNoNaNs(bool B);

  /// Set or clear the no-infs flag on this instruction, which must be an
  /// operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasNoInfs(bool B);

  /// Set or clear the no-signed-zeros flag on this instruction, which must be
  /// an operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasNoSignedZeros(bool B);

  /// Set or clear the allow-reciprocal flag on this instruction, which must be
  /// an operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasAllowReciprocal(bool B);

  /// Set or clear the allow-contract flag on this instruction, which must be
  /// an operator which supports this flag. See LangRef.html for the meaning of
  /// this flag.
  LLVM_CORE_ABI void setHasAllowContract(bool B);

  /// Set or clear the approximate-math-functions flag on this instruction,
  /// which must be an operator which supports this flag. See LangRef.html for
  /// the meaning of this flag.
  LLVM_CORE_ABI void setHasApproxFunc(bool B);

  /// Convenience function for setting multiple fast-math flags on this
  /// instruction, which must be an operator which supports these flags. See
  /// LangRef.html for the meaning of these flags.
  LLVM_CORE_ABI void setFastMathFlags(FastMathFlags FMF);

  /// Convenience function for transferring all fast-math flag values to this
  /// instruction, which must be an operator which supports these flags. See
  /// LangRef.html for the meaning of these flags.
  LLVM_CORE_ABI void copyFastMathFlags(FastMathFlags FMF);

  /// Determine whether all fast-math-flags are set.
  LLVM_CORE_ABI bool isFast() const LLVM_READONLY;

  /// Determine whether the allow-reassociation flag is set.
  LLVM_CORE_ABI bool hasAllowReassoc() const LLVM_READONLY;

  /// Determine whether the no-NaNs flag is set.
  LLVM_CORE_ABI bool hasNoNaNs() const LLVM_READONLY;

  /// Determine whether the no-infs flag is set.
  LLVM_CORE_ABI bool hasNoInfs() const LLVM_READONLY;

  /// Determine whether the no-signed-zeros flag is set.
  LLVM_CORE_ABI bool hasNoSignedZeros() const LLVM_READONLY;

  /// Determine whether the allow-reciprocal flag is set.
  LLVM_CORE_ABI bool hasAllowReciprocal() const LLVM_READONLY;

  /// Determine whether the allow-contract flag is set.
  LLVM_CORE_ABI bool hasAllowContract() const LLVM_READONLY;

  /// Determine whether the approximate-math-functions flag is set.
  LLVM_CORE_ABI bool hasApproxFunc() const LLVM_READONLY;

  /// Convenience function for getting all the fast-math flags, which must be an
  /// operator which supports these flags. See LangRef.html for the meaning of
  /// these flags.
  LLVM_CORE_ABI FastMathFlags getFastMathFlags() const LLVM_READONLY;

  /// Copy I's fast-math flags
  LLVM_CORE_ABI void copyFastMathFlags(const Instruction *I);

  /// Convenience method to copy supported exact, fast-math, and (optionally)
  /// wrapping flags from V to this instruction.
  LLVM_CORE_ABI void copyIRFlags(const Value *V, bool IncludeWrapFlags = true);

  /// Logical 'and' of any supported wrapping, exact, and fast-math flags of
  /// V and this instruction.
  LLVM_CORE_ABI void andIRFlags(const Value *V);
  ...
  /// merged DebugLoc.
  LLVM_CORE_ABI void applyMergedLocation(DILocation *LocA, DILocation *LocB);

  /// Updates the debug location given that the instruction has been hoisted
  /// from a block to a predecessor of that block.
  /// Note: it is undefined behavior to call this on an instruction not
  /// currently inserted into a function.
  LLVM_CORE_ABI void updateLocationAfterHoist();
  ...
  /// Note: it is undefined behavior to call this on an instruction not
  /// currently inserted into a function.
  LLVM_CORE_ABI void dropLocation();
...
private:
  ...
  LLVM_CORE_ABI MDNode *getMetadataImpl(StringRef Kind) const;
  LLVM_CORE_ABI void
  ...
  LLVM_CORE_ABI void updateDIAssignIDMapping(DIAssignID *ID);
...
public:
  ...
  ///
  LLVM_CORE_ABI bool isAssociative() const LLVM_READONLY;
  ...
  /// applied to any type.
  ///
  LLVM_CORE_ABI bool isCommutative() const LLVM_READONLY;
  ...
  LLVM_CORE_ABI bool mayWriteToMemory() const LLVM_READONLY;

  /// Return true if this instruction may read memory.
  LLVM_CORE_ABI bool mayReadFromMemory() const LLVM_READONLY;
  ...
  /// higher.
  LLVM_CORE_ABI bool isAtomic() const LLVM_READONLY;

  /// Return true if this atomic instruction loads from memory.
  LLVM_CORE_ABI bool hasAtomicLoad() const LLVM_READONLY;

  /// Return true if this atomic instruction stores to memory.
  LLVM_CORE_ABI bool hasAtomicStore() const LLVM_READONLY;

  /// Return true if this instruction has a volatile memory access.
  LLVM_CORE_ABI bool isVolatile() const LLVM_READONLY;

  /// Return the type this instruction accesses in memory, if any.
  LLVM_CORE_ABI Type *getAccessType() const LLVM_READONLY;
  ...
  LLVM_CORE_ABI bool mayHaveSideEffects() const LLVM_READONLY;
  ...
  LLVM_CORE_ABI bool isLifetimeStartOrEnd() const LLVM_READONLY;

  /// Return true if the instruction is a llvm.launder.invariant.group or
  /// llvm.strip.invariant.group.
  LLVM_CORE_ABI bool isLaunderOrStripInvariantGroup() const LLVM_READONLY;

  /// Return true if the instruction is a DbgInfoIntrinsic or PseudoProbeInst.
  LLVM_CORE_ABI bool isDebugOrPseudoInst() const LLVM_READONLY;

  /// Return a pointer to the next non-debug instruction in the same basic
  /// block as 'this', or nullptr if no such instruction exists. Skip any pseudo
  /// operations if \c SkipPseudoOp is true.
  LLVM_CORE_ABI const Instruction *
  ...
  ///
  LLVM_CORE_ABI Instruction *clone() const;
  ...
  LLVM_CORE_ABI bool isIdenticalTo(const Instruction *I) const LLVM_READONLY;
  ...
  LLVM_CORE_ABI bool isSameOperationAs(const Instruction *I,
  ...
  LLVM_CORE_ABI unsigned getNumSuccessors() const LLVM_READONLY;

  /// Return the specified successor. This instruction must be a terminator.
  LLVM_CORE_ABI BasicBlock *getSuccessor(unsigned Idx) const LLVM_READONLY;

  /// Update the specified successor to point at the provided block. This
  /// instruction must be a terminator.
  LLVM_CORE_ABI void setSuccessor(unsigned Idx, BasicBlock *BB);

  /// Replace specified successor OldBB to point at the provided block.
  /// This instruction must be a terminator.
  LLVM_CORE_ABI void replaceSuccessorWith(BasicBlock *OldBB, BasicBlock *NewBB);
...
protected:
  ...
  LLVM_CORE_ABI Instruction(Type *Ty, unsigned iType, AllocInfo AllocInfo,
                            InsertPosition InsertBefore = nullptr);

private:
  /// Create a copy of this instruction.
  LLVM_CORE_ABI Instruction *cloneImpl() const;
};

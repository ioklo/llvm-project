...
#undef LLVM_FIXED_MD_KIND
  ...
  /// name TagName.
  LLVM_CORE_ABI StringMapEntry<uint32_t> *
  getOrInsertBundleTag(StringRef TagName) const;

  /// getOperandBundleTagID - Maps a bundle tag to an integer ID.  Every bundle
  /// tag registered with an LLVMContext has an unique ID.
  LLVM_CORE_ABI uint32_t getOperandBundleTagID(StringRef Tag) const;

  /// getOrInsertSyncScopeID - Maps synchronization scope name to
  /// synchronization scope ID.  Every synchronization scope registered with
  /// LLVMContext has unique ID except pre-defined ones.
  LLVM_CORE_ABI SyncScope::ID getOrInsertSyncScopeID(StringRef SSN);

  /// getSyncScopeNames - Populates client supplied SmallVector with
  /// synchronization scope names registered with LLVMContext.  Synchronization
  /// scope names are ordered by increasing synchronization scope IDs.
  LLVM_CORE_ABI void getSyncScopeNames(SmallVectorImpl<StringRef> &SSNs) const;

  /// getSyncScopeName - Returns the name of a SyncScope::ID
  /// registered with LLVMContext, if any.
  LLVM_CORE_ABI std::optional<StringRef>
  getSyncScopeName(SyncScope::ID Id) const;

  /// Define the GC for a function
  LLVM_CORE_ABI void setGC(const Function &Fn, std::string GCName);

  /// Return the GC for a function
  LLVM_CORE_ABI const std::string &getGC(const Function &Fn);

  /// Remove the GC for a function
  LLVM_CORE_ABI void deleteGC(const Function &Fn);

  /// Return true if the Context runtime configuration is set to discard all
  /// value names. When true, only GlobalValue names will be available in the
  /// IR.
  LLVM_CORE_ABI bool shouldDiscardValueNames() const;

  /// Set the Context runtime configuration to discard all value name (but
  /// GlobalValue). Clients can use this flag to save memory and runtime,
  /// especially in release mode.
  LLVM_CORE_ABI void setDiscardValueNames(bool Discard);

  /// Whether there is a string map for uniquing debug info
  /// identifiers across the context.  Off by default.
  LLVM_CORE_ABI bool isODRUniquingDebugTypes() const;
  LLVM_CORE_ABI void enableDebugTypeODRUniquing();
  LLVM_CORE_ABI void disableDebugTypeODRUniquing();

  /// generateMachineFunctionNum - Get a unique number for MachineFunction
  /// that associated with the given Function.
  LLVM_CORE_ABI unsigned generateMachineFunctionNum(Function &);

  /// Defines the type of a yield callback.
  /// \see LLVMContext::setYieldCallback.
  using YieldCallbackTy =
      void(*)(LLVMContext * Context, void *OpaqueHandle);
  ...
  /// pointers.
  LLVM_CORE_ABI void setDiagnosticHandlerCallBack(
  ...
  LLVM_CORE_ABI void
  setDiagnosticHandler(std::unique_ptr<DiagnosticHandler> &&DH,
                       bool RespectFilters = false);

  /// getDiagnosticHandlerCallBack - Return the diagnostic handler call back set
  /// by setDiagnosticHandlerCallBack.
  LLVM_CORE_ABI DiagnosticHandler::DiagnosticHandlerTy
  getDiagnosticHandlerCallBack() const;

  /// getDiagnosticContext - Return the diagnostic context set by
  /// setDiagnosticContext.
  LLVM_CORE_ABI void *getDiagnosticContext() const;

  /// getDiagHandlerPtr - Returns const raw pointer of DiagnosticHandler set by
  /// setDiagnosticHandler.
  LLVM_CORE_ABI const DiagnosticHandler *getDiagHandlerPtr() const;

  /// getDiagnosticHandler - transfers ownership of DiagnosticHandler unique_ptr
  /// to caller.
  LLVM_CORE_ABI std::unique_ptr<DiagnosticHandler> getDiagnosticHandler();

  /// Return if a code hotness metric should be included in optimization
  /// diagnostics.
  LLVM_CORE_ABI bool getDiagnosticsHotnessRequested() const;
  /// Set if a code hotness metric should be included in optimization
  /// diagnostics.
  LLVM_CORE_ABI void setDiagnosticsHotnessRequested(bool Requested);

  LLVM_CORE_ABI bool getMisExpectWarningRequested() const;
  LLVM_CORE_ABI void setMisExpectWarningRequested(bool Requested);
  LLVM_CORE_ABI void
  setDiagnosticsMisExpectTolerance(std::optional<uint32_t> Tolerance);
  LLVM_CORE_ABI uint32_t getDiagnosticsMisExpectTolerance() const;
  ...
  LLVM_CORE_ABI uint64_t getDiagnosticsHotnessThreshold() const;
  ...
  LLVM_CORE_ABI bool isDiagnosticsHotnessThresholdSetFromPSI() const;
  ...
  LLVM_CORE_ABI remarks::RemarkStreamer *getMainRemarkStreamer();
  LLVM_CORE_ABI const remarks::RemarkStreamer *getMainRemarkStreamer() const;
  LLVM_CORE_ABI void setMainRemarkStreamer(
  ...
  LLVM_CORE_ABI LLVMRemarkStreamer *getLLVMRemarkStreamer();
  LLVM_CORE_ABI const LLVMRemarkStreamer *getLLVMRemarkStreamer() const;
  LLVM_CORE_ABI void
  setLLVMRemarkStreamer(std::unique_ptr<LLVMRemarkStreamer> RemarkStreamer);

  /// Get the prefix that should be printed in front of a diagnostic of
  ///        the given \p Severity
  LLVM_CORE_ABI static const char *
  ...
  LLVM_CORE_ABI void diagnose(const DiagnosticInfo &DI);
  ...
  LLVM_CORE_ABI void setYieldCallback(YieldCallbackTy Callback,
                                      void *OpaqueHandle);
  ...
  LLVM_CORE_ABI void emitError(const Instruction *I, const Twine &ErrorStr);
  LLVM_CORE_ABI void emitError(const Twine &ErrorStr);

  /// Access the object which can disable optional passes and individual
  /// optimizations at compile time.
  LLVM_CORE_ABI OptPassGate &getOptPassGate() const;
  ...
  LLVM_CORE_ABI void setOptPassGate(OptPassGate &);
  ...
  LLVM_CORE_ABI StringRef getDefaultTargetCPU();
  LLVM_CORE_ABI void setDefaultTargetCPU(StringRef CPU);

  /// Similar to {get,set}DefaultTargetCPU() but for default target-features.
  LLVM_CORE_ABI StringRef getDefaultTargetFeatures();
  LLVM_CORE_ABI void setDefaultTargetFeatures(StringRef Features);
...
private:
  ...
  LLVM_CORE_ABI void addModule(Module *);

  /// removeModule - Unregister a module from this context.
  LLVM_CORE_ABI void removeModule(Module *);
};

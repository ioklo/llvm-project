...
LLVM_CORE_ABI TinyPtrVector<DbgDeclareInst *> findDbgDeclares(Value *V);
/// As above, for DVRDeclares.
LLVM_CORE_ABI TinyPtrVector<DbgVariableRecord *> findDVRDeclares(Value *V);
/// As above, for DVRValues.
LLVM_CORE_ABI TinyPtrVector<DbgVariableRecord *> findDVRValues(Value *V);

/// Finds the llvm.dbg.value intrinsics describing a value.
LLVM_CORE_ABI void findDbgValues(
    SmallVectorImpl<DbgValueInst *> &DbgValues, Value *V,
    SmallVectorImpl<DbgVariableRecord *> *DbgVariableRecords = nullptr);

/// Finds the debug info intrinsics describing a value.
LLVM_CORE_ABI void findDbgUsers(
    SmallVectorImpl<DbgVariableIntrinsic *> &DbgInsts, Value *V,
    SmallVectorImpl<DbgVariableRecord *> *DbgVariableRecords = nullptr);

/// Find subprogram that is enclosing this scope.
LLVM_CORE_ABI DISubprogram *getDISubprogram(const MDNode *Scope);

/// Produce a DebugLoc to use for each dbg.declare that is promoted to a
/// dbg.value.
LLVM_CORE_ABI DebugLoc getDebugValueLoc(DbgVariableIntrinsic *DII);
LLVM_CORE_ABI DebugLoc getDebugValueLoc(DbgVariableRecord *DVR);
...
LLVM_CORE_ABI bool StripDebugInfo(Module &M);
LLVM_CORE_ABI bool stripDebugInfo(Function &F);
...
LLVM_CORE_ABI bool stripNonLineTableDebugInfo(Module &M);
...
LLVM_CORE_ABI unsigned getDebugMetadataVersionFromModule(const Module &M);
...
public:
  ...
  LLVM_CORE_ABI void processModule(const Module &M);
  /// Process a single instruction and collect debug info anchors.
  LLVM_CORE_ABI void processInstruction(const Module &M, const Instruction &I);

  /// Process a DILocalVariable.
  LLVM_CORE_ABI void processVariable(const Module &M,
                                     const DILocalVariable *DVI);
  /// Process debug info location.
  LLVM_CORE_ABI void processLocation(const Module &M, const DILocation *Loc);
  /// Process a DbgRecord (e.g, treat a DbgVariableRecord like a
  /// DbgVariableIntrinsic).
  LLVM_CORE_ABI void processDbgRecord(const Module &M, const DbgRecord &DR);

  /// Process subprogram.
  LLVM_CORE_ABI void processSubprogram(DISubprogram *SP);

  /// Clear all lists.
  LLVM_CORE_ABI void reset();

private:
  LLVM_CORE_ABI void processCompileUnit(DICompileUnit *CU);
  LLVM_CORE_ABI void processScope(DIScope *Scope);
  LLVM_CORE_ABI void processType(DIType *DT);
  LLVM_CORE_ABI bool addCompileUnit(DICompileUnit *CU);
  LLVM_CORE_ABI bool addGlobalVariable(DIGlobalVariableExpression *DIG);
  LLVM_CORE_ABI bool addScope(DIScope *Scope);
  LLVM_CORE_ABI bool addSubprogram(DISubprogram *SP);
  LLVM_CORE_ABI bool addType(DIType *DT);
...
LLVM_CORE_ABI AssignmentInstRange getAssignmentInsts(DIAssignID *ID);
...
LLVM_CORE_ABI AssignmentMarkerRange getAssignmentMarkers(DIAssignID *ID);
...
LLVM_CORE_ABI void deleteAssignmentMarkers(const Instruction *Inst);

/// Replace all uses (and attachments) of \p Old with \p New.
LLVM_CORE_ABI void RAUW(DIAssignID *Old, DIAssignID *New);

/// Remove all Assignment Tracking related intrinsics and metadata from \p F.
LLVM_CORE_ABI void deleteAll(Function *F);
...
LLVM_CORE_ABI void remapAssignID(DenseMap<DIAssignID *, DIAssignID *> &Map,
                                 Instruction &I);
...
LLVM_CORE_ABI void trackAssignments(Function::iterator Start, Function::iterator End,
...
LLVM_CORE_ABI std::optional<AssignmentInfo> getAssignmentInfo(const DataLayout &DL,
...
LLVM_CORE_ABI bool isAssignmentTrackingEnabled(const Module &M);

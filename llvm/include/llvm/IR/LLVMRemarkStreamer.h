...
class LLVMRemarkStreamer {
  ...
  LLVM_CORE_ABI remarks::Remark toRemark(const DiagnosticInfoOptimizationBase &Diag) const;

public:
  LLVMRemarkStreamer(remarks::RemarkStreamer &RS) : RS(RS) {}
  /// Emit a diagnostic through the streamer.
  LLVM_CORE_ABI void emit(const DiagnosticInfoOptimizationBase &Diag);
};
...
LLVM_CORE_ABI Expected<std::unique_ptr<ToolOutputFile>>
...
LLVM_CORE_ABI Error setupLLVMOptimizationRemarks(

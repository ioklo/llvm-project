...
public:
  ...
  LLVM_OBJECT_ABI static Expected<std::unique_ptr<OffloadBinary>>
      create(MemoryBufferRef);

  /// Serialize the contents of \p File to a binary buffer to be read later.
  LLVM_OBJECT_ABI static SmallString<0> write(const OffloadingImage &);
...
/// of \p Binaries.
LLVM_OBJECT_ABI Error extractOffloadBinaries(
    MemoryBufferRef Buffer, SmallVectorImpl<OffloadFile> &Binaries);

/// Convert a string \p Name to an image kind.
LLVM_OBJECT_ABI ImageKind getImageKind(StringRef Name);

/// Convert an image kind to its string representation.
LLVM_OBJECT_ABI StringRef getImageKindName(ImageKind Name);

/// Convert a string \p Name to an offload kind.
LLVM_OBJECT_ABI OffloadKind getOffloadKind(StringRef Name);

/// Convert an offload kind to its string representation.
LLVM_OBJECT_ABI StringRef getOffloadKindName(OffloadKind Name);
...
LLVM_OBJECT_ABI bool areTargetsCompatible(const OffloadFile::TargetID &LHS,

...
struct NewArchiveMember {
  ...
  LLVM_OBJECT_ABI NewArchiveMember(MemoryBufferRef BufRef);
  ...
  LLVM_OBJECT_ABI object::Archive::Kind detectKindFromObject() const;

  LLVM_OBJECT_ABI static Expected<NewArchiveMember>
  getOldMember(const object::Archive::Child &OldMember, bool Deterministic);

  LLVM_OBJECT_ABI static Expected<NewArchiveMember> getFile(StringRef FileName,
                                                            bool Deterministic);
};

LLVM_OBJECT_ABI Expected<std::string> computeArchiveRelativePath(StringRef From,
                                                                 StringRef To);
...
LLVM_OBJECT_ABI void warnToStderr(Error Err);

// Write an archive directly to an output stream.
LLVM_OBJECT_ABI Error writeArchiveToStream(

...
public:
  ...
  /// result (see result()).
  LLVM_SUPPORT_ABI std::array<uint8_t, 32> final();
  ...
  LLVM_SUPPORT_ABI std::array<uint8_t, 32> result();

  /// Returns a raw 256-bit SHA256 hash for the given data.
  LLVM_SUPPORT_ABI static std::array<uint8_t, 32> hash(ArrayRef<uint8_t> Data);

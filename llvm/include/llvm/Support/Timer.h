...
public:
  ...
  LLVM_SUPPORT_ABI static TimeRecord getCurrentTime(bool Start = true);
  ...
  LLVM_SUPPORT_ABI void print(const TimeRecord &Total, raw_ostream &OS) const;
};
...
public:
  ...
  LLVM_SUPPORT_ABI ~Timer();

  /// Create an uninitialized timer, client must use 'init'.
  explicit Timer() = default;
  LLVM_SUPPORT_ABI void init(StringRef TimerName, StringRef TimerDescription);
  LLVM_SUPPORT_ABI void init(StringRef TimerName, StringRef TimerDescription,
                             TimerGroup &tg);
  ...
  /// paired.
  LLVM_SUPPORT_ABI void startTimer();

  /// Stop the timer.
  LLVM_SUPPORT_ABI void stopTimer();

  /// Clear the timer state.
  LLVM_SUPPORT_ABI void clear();

  /// Stop the timer and start another timer.
  LLVM_SUPPORT_ABI void yieldTo(Timer &);
...
struct NamedRegionTimer : public TimeRegion {
  LLVM_SUPPORT_ABI explicit NamedRegionTimer(StringRef Name,
...
public:
  LLVM_SUPPORT_ABI explicit TimerGroup(StringRef Name, StringRef Description);

  LLVM_SUPPORT_ABI explicit TimerGroup(StringRef Name, StringRef Description,
                                       const StringMap<TimeRecord> &Records);

  LLVM_SUPPORT_ABI ~TimerGroup();
  ...
  /// printing them.
  LLVM_SUPPORT_ABI void print(raw_ostream &OS, bool ResetAfterPrint = false);

  /// Clear all timers in this group.
  LLVM_SUPPORT_ABI void clear();

  /// This static method prints all timers.
  LLVM_SUPPORT_ABI static void printAll(raw_ostream &OS);
  ...
  LLVM_SUPPORT_ABI static void clearAll();

  LLVM_SUPPORT_ABI const char *printJSONValues(raw_ostream &OS,
                                               const char *delim);

  /// Prints all timers as JSON key/value pairs.
  LLVM_SUPPORT_ABI static const char *printAllJSONValues(raw_ostream &OS,
                                                         const char *delim);
  ...
  LLVM_SUPPORT_ABI static void constructForStatistics();

  /// This makes the timer globals unmanaged, and lets the user manage the
  /// lifetime.
  LLVM_SUPPORT_ABI static void *acquireTimerGlobals();

...
public:
  ...
  /// (see result()).
  LLVM_SUPPORT_ABI std::array<uint8_t, 20> final();
  ...
  /// made into update.
  LLVM_SUPPORT_ABI std::array<uint8_t, 20> result();

  /// Returns a raw 160-bit SHA1 hash for the given data.
  LLVM_SUPPORT_ABI static std::array<uint8_t, 20> hash(ArrayRef<uint8_t> Data);

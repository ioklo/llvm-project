...
private:
  struct SrcBuffer {
    ...
    /// from.
    LLVM_SUPPORT_ABI unsigned getLineNumber(const char *Ptr) const;
    template <typename T>
    unsigned getLineNumberSpecialized(const char *Ptr) const;

    /// Return a pointer to the first character of the specified line number or
    /// null if the line number is invalid.
    LLVM_SUPPORT_ABI const char *getPointerForLineNumber(unsigned LineNo) const;
    template <typename T>
    ...
    SrcBuffer() = default;
    LLVM_SUPPORT_ABI SrcBuffer(SrcBuffer &&);
    SrcBuffer(const SrcBuffer &) = delete;
    SrcBuffer &operator=(const SrcBuffer &) = delete;
    LLVM_SUPPORT_ABI ~SrcBuffer();
  };
...
public:
  ...
  /// \p IncludedFile.
  LLVM_SUPPORT_ABI unsigned AddIncludeFile(const std::string &Filename,
                                           SMLoc IncludeLoc,
  ...
  LLVM_SUPPORT_ABI ErrorOr<std::unique_ptr<MemoryBuffer>>
  OpenIncludeFile(const std::string &Filename, std::string &IncludedFile);

  /// Return the ID of the buffer containing the specified location.
  ///
  /// 0 is returned if the buffer is not found.
  LLVM_SUPPORT_ABI unsigned FindBufferContainingLoc(SMLoc Loc) const;
  ...
  LLVM_SUPPORT_ABI std::pair<unsigned, unsigned>
  ...
  LLVM_SUPPORT_ABI void PrintMessage(raw_ostream &OS, SMLoc Loc, DiagKind Kind,
                                     const Twine &Msg,
  ...
  LLVM_SUPPORT_ABI void PrintMessage(SMLoc Loc, DiagKind Kind, const Twine &Msg,
                                     ArrayRef<SMRange> Ranges = {},
                                     ArrayRef<SMFixIt> FixIts = {},
                                     bool ShowColors = true) const;
  ...
  /// \param ShowColors Display colored messages if output is a terminal and
  /// the default error handler is used.
  LLVM_SUPPORT_ABI void PrintMessage(raw_ostream &OS,
  ...
  LLVM_SUPPORT_ABI SMDiagnostic GetMessage(SMLoc Loc, DiagKind Kind,
                                           const Twine &Msg,
                                           ArrayRef<SMRange> Ranges = {},
  ...
  LLVM_SUPPORT_ABI void PrintIncludeStack(SMLoc IncludeLoc,
...
public:
  LLVM_SUPPORT_ABI SMFixIt(SMRange R, const Twine &Replacement);
...
public:
  ...
  LLVM_SUPPORT_ABI SMDiagnostic(const SourceMgr &sm, SMLoc L, StringRef FN,
  ...
  LLVM_SUPPORT_ABI void print(const char *ProgName, raw_ostream &S,

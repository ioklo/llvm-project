...
public:
  ...
  LLVM_SUPPORT_ABI static std::unique_ptr<SpecialCaseList>
  ...
  LLVM_SUPPORT_ABI ~SpecialCaseList();
  ...
  LLVM_SUPPORT_ABI bool inSection(StringRef Section, StringRef Prefix,
                                  StringRef Query,
...
protected:
  ...
  // classes.
  LLVM_SUPPORT_ABI bool createInternal(const std::vector<std::string> &Paths,
                                       vfs::FileSystem &VFS,
                                       std::string &Error);
  LLVM_SUPPORT_ABI bool createInternal(const MemoryBuffer *MB,
                                       std::string &Error);
  ...
  class Matcher {
  public:
    LLVM_SUPPORT_ABI Error insert(StringRef Pattern, unsigned LineNumber,
                                  bool UseRegex);
  ...
  LLVM_SUPPORT_ABI Expected<Section *>
  addSection(StringRef SectionStr, unsigned LineNo, bool UseGlobs = true);

  /// Parses just-constructed SpecialCaseList entries from a memory buffer.
  LLVM_SUPPORT_ABI bool parse(const MemoryBuffer *MB, std::string &Error);

  // Helper method for derived classes to search by Prefix, Query, and Category
  // once they have already resolved a section entry.
  LLVM_SUPPORT_ABI unsigned inSectionBlame(const SectionEntries &Entries,

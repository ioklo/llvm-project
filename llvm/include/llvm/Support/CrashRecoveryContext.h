...
public:
  LLVM_SUPPORT_ABI CrashRecoveryContext();
  LLVM_SUPPORT_ABI ~CrashRecoveryContext();

  /// Register cleanup handler, which is used when the recovery context is
  /// finished.
  /// The recovery context owns the handler.
  LLVM_SUPPORT_ABI void registerCleanup(CrashRecoveryContextCleanup *cleanup);

  LLVM_SUPPORT_ABI void unregisterCleanup(CrashRecoveryContextCleanup *cleanup);

  /// Enable crash recovery.
  LLVM_SUPPORT_ABI static void Enable();

  /// Disable crash recovery.
  LLVM_SUPPORT_ABI static void Disable();

  /// Return the active context, if the code is currently executing in a
  /// thread which is in a protected context.
  LLVM_SUPPORT_ABI static CrashRecoveryContext *GetCurrent();

  /// Return true if the current thread is recovering from a crash.
  LLVM_SUPPORT_ABI static bool isRecoveringFromCrash();
  ...
  LLVM_SUPPORT_ABI bool RunSafely(function_ref<void()> Fn);
  ...
  LLVM_SUPPORT_ABI bool RunSafelyOnThread(function_ref<void()>,
  ...
  [[noreturn]] LLVM_SUPPORT_ABI void HandleExit(int RetCode);

  /// Return true if RetCode indicates that a signal or an exception occurred.
  LLVM_SUPPORT_ABI static bool isCrash(int RetCode);

  /// Throw again a signal or an exception, after it was catched once by a
  /// CrashRecoveryContext.
  LLVM_SUPPORT_ABI static bool throwIfCrash(int RetCode);
...
public:
  ...
  LLVM_SUPPORT_ABI virtual ~CrashRecoveryContextCleanup();

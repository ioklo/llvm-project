...
namespace llvm {
  ...
  public:
    ...
    LLVM_SUPPORT_ABI Regex(StringRef Regex, RegexFlags Flags = NoFlags);
    LLVM_SUPPORT_ABI Regex(StringRef Regex, unsigned Flags);
    ...
    LLVM_SUPPORT_ABI Regex(Regex &&regex);
    LLVM_SUPPORT_ABI ~Regex();

    /// isValid - returns the error encountered during regex compilation, if
    /// any.
    LLVM_SUPPORT_ABI bool isValid(std::string &Error) const;
    bool isValid() const { return !error; }

    /// getNumMatches - In a valid regex, return the number of parenthesized
    /// matches it contains.  The number filled in by match will include this
    /// many entries plus one for the whole regex (as element 0).
    LLVM_SUPPORT_ABI unsigned getNumMatches() const;
    ...
    LLVM_SUPPORT_ABI bool match(StringRef String,
    ...
    LLVM_SUPPORT_ABI std::string sub(StringRef Repl, StringRef String,
                                     std::string *Error = nullptr) const;

    /// If this function returns true, ^Str$ is an extended regular
    /// expression that matches Str and only Str.
    LLVM_SUPPORT_ABI static bool isLiteralERE(StringRef Str);

    /// Turn String into a regex by escaping its special characters.
    LLVM_SUPPORT_ABI static std::string escape(StringRef String);

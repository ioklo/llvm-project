...
public:
  struct MD5Result : public std::array<uint8_t, 16> {
    LLVM_SUPPORT_ABI SmallString<32> digest() const;
  ...
  LLVM_SUPPORT_ABI MD5();

  /// Updates the hash for the byte stream provided.
  LLVM_SUPPORT_ABI void update(ArrayRef<uint8_t> Data);

  /// Updates the hash for the StringRef provided.
  LLVM_SUPPORT_ABI void update(StringRef Str);

  /// Finishes off the hash and puts the result in result.
  LLVM_SUPPORT_ABI void final(MD5Result &Result);

  /// Finishes off the hash, and returns the 16-byte hash data.
  LLVM_SUPPORT_ABI MD5Result final();
  ...
  LLVM_SUPPORT_ABI MD5Result result();

  /// Translates the bytes in \p Res to a hex string that is
  /// deposited into \p Str. The result will be of length 32.
  LLVM_SUPPORT_ABI static void stringifyResult(MD5Result &Result,
                                               SmallVectorImpl<char> &Str);

  /// Computes the hash for a given bytes.
  LLVM_SUPPORT_ABI static MD5Result hash(ArrayRef<uint8_t> Data);
...
private:
  ...
  LLVM_SUPPORT_ABI const uint8_t *body(ArrayRef<uint8_t> Data);
};

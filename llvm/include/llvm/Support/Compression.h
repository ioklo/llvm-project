...
LLVM_SUPPORT_ABI void compress(ArrayRef<uint8_t> Input,
                               SmallVectorImpl<uint8_t> &CompressedBuffer,
                               int Level = DefaultCompression);

LLVM_SUPPORT_ABI Error decompress(ArrayRef<uint8_t> Input, uint8_t *Output,
                                  size_t &UncompressedSize);

LLVM_SUPPORT_ABI Error decompress(ArrayRef<uint8_t> Input,
...
LLVM_SUPPORT_ABI const char *getReasonIfUnsupported(Format F);

// Compress Input with the specified format P.Format. If Level is -1, use
// *::DefaultCompression for the format.
LLVM_SUPPORT_ABI void compress(Params P, ArrayRef<uint8_t> Input,
              SmallVectorImpl<uint8_t> &Output);

// Decompress Input. The uncompressed size must be available.
LLVM_SUPPORT_ABI Error decompress(DebugCompressionType T,
                                  ArrayRef<uint8_t> Input,
                                  uint8_t *Output, size_t UncompressedSize);
LLVM_SUPPORT_ABI Error decompress(Format F, ArrayRef<uint8_t> Input,

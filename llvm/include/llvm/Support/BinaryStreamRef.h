...
public:
  ...
  LLVM_SUPPORT_ABI BinaryStreamRef(BinaryStream &Stream);
  LLVM_SUPPORT_ABI BinaryStreamRef(BinaryStream &Stream, uint64_t Offset,
                                   std::optional<uint64_t> Length);
  LLVM_SUPPORT_ABI explicit BinaryStreamRef(ArrayRef<uint8_t> Data,
                                            llvm::endianness Endian);
  LLVM_SUPPORT_ABI explicit BinaryStreamRef(StringRef Data,
                                            llvm::endianness Endian);
  ...
  LLVM_SUPPORT_ABI Error readBytes(uint64_t Offset, uint64_t Size,
  ...
  readLongestContiguousChunk(uint64_t Offset, ArrayRef<uint8_t> &Buffer) const;
};
...
public:
  ...
  LLVM_SUPPORT_ABI WritableBinaryStreamRef(WritableBinaryStream &Stream);
  LLVM_SUPPORT_ABI WritableBinaryStreamRef(WritableBinaryStream &Stream,
                                           uint64_t Offset,
                                           std::optional<uint64_t> Length);
  LLVM_SUPPORT_ABI explicit WritableBinaryStreamRef(
  ...
  LLVM_SUPPORT_ABI Error writeBytes(uint64_t Offset,
                                    ArrayRef<uint8_t> Data) const;

  /// Conver this WritableBinaryStreamRef to a read-only BinaryStreamRef.
  LLVM_SUPPORT_ABI operator BinaryStreamRef() const;

  /// For buffered streams, commits changes to the backing store.
  LLVM_SUPPORT_ABI Error commit();
};

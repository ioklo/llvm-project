...
public:
  ...
  LLVM_SUPPORT_ABI TimePoint<> getLastAccessedTime() const;
  ...
  /// same machine.
  LLVM_SUPPORT_ABI TimePoint<> getLastModificationTime() const;
...
public:
  ...
  LLVM_SUPPORT_ABI UniqueID getUniqueID() const;
  LLVM_SUPPORT_ABI uint32_t getLinkCount() const;
};
...
LLVM_SUPPORT_ABI void make_absolute(const Twine &current_directory,
...
create_directories(const Twine &path,
...
///          error if the directory already existed.
LLVM_SUPPORT_ABI std::error_code
create_directory(const Twine &path, bool IgnoreExisting = true,
                 perms Perms = owner_all | group_all);
...
/// specific error_code.
LLVM_SUPPORT_ABI std::error_code create_link(const Twine &to,
                                             const Twine &from);
...
/// @returns errc::success if the link was created, otherwise a platform
/// specific error_code.
LLVM_SUPPORT_ABI std::error_code create_hard_link(const Twine &to,
                                                  const Twine &from);
...
///                     directory.
LLVM_SUPPORT_ABI std::error_code real_path(const Twine &path,
...
///
/// @param path The path to resolve.
LLVM_SUPPORT_ABI void expand_tilde(const Twine &path,
                                   SmallVectorImpl<char> &output);
...
LLVM_SUPPORT_ABI std::error_code remove(const Twine &path,
...
LLVM_SUPPORT_ABI std::error_code remove_directories(const Twine &path,
...
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code equivalent(const Twine &A, const Twine &B,
                                            bool &result);
...
LLVM_SUPPORT_ABI file_type get_file_type(const Twine &Path, bool Follow = true);
...
/// @returns errc::success if result has been successfully set, otherwise a
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code is_regular_file(const Twine &path,
                                                 bool &result);
...
LLVM_SUPPORT_ABI bool is_symlink_file(const basic_file_status &status);
...
/// @returns errc::success if result has been successfully set, otherwise a
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code is_symlink_file(const Twine &path,
                                                 bool &result);
...
LLVM_SUPPORT_ABI bool is_other(const basic_file_status &status);
...
/// @returns errc::success if result has been successfully set, otherwise a
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code is_other(const Twine &path, bool &result);
...
LLVM_SUPPORT_ABI std::error_code setPermissions(const Twine &Path,
                                                perms Permissions);
...
LLVM_SUPPORT_ABI bool status_known(const basic_file_status &s);
...
/// @returns errc::success if result has been successfully set, otherwise a
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code status_known(const Twine &path, bool &result);
...
LLVM_SUPPORT_ABI void createUniquePath(const Twine &Model,
...
///          otherwise a platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code
createUniqueFile(const Twine &Model, int &ResultFD,
                 SmallVectorImpl<char> &ResultPath,
...
LLVM_SUPPORT_ABI std::error_code
createUniqueFile(const Twine &Model,
                 SmallVectorImpl<char> &ResultPath,
                 unsigned Mode = all_read | all_write);
...
public:
  ...
  LLVM_SUPPORT_ABI static Expected<TempFile>
  create(const Twine &Model,
         unsigned Mode = all_read | all_write,
         OpenFlags ExtraFlags = OF_None);
  LLVM_SUPPORT_ABI TempFile(TempFile &&Other);
  LLVM_SUPPORT_ABI TempFile &operator=(TempFile &&Other);
  ...
  LLVM_SUPPORT_ABI Error keep(const Twine &Name);

  // Keep this with the temporary name.
  LLVM_SUPPORT_ABI Error keep();

  // Delete the file.
  LLVM_SUPPORT_ABI Error discard();

  // This checks that keep or delete was called.
  LLVM_SUPPORT_ABI ~TempFile();
};
...
LLVM_SUPPORT_ABI std::error_code
createTemporaryFile(const Twine &Prefix, StringRef Suffix,
                    int &ResultFD,
                    SmallVectorImpl<char> &ResultPath,
                    OpenFlags Flags = OF_None);
...
///          platform-specific error_code.
LLVM_SUPPORT_ABI std::error_code 
openFile(const Twine &Name, int &ResultFD,
...
openNativeFile(const Twine &Name, CreationDisposition Disp,
...
LLVM_SUPPORT_ABI Expected<size_t> readNativeFile(file_t FileHandle,
...
readNativeFileToEOF(file_t FileHandle, SmallVectorImpl<char> &Buffer,
...
/// @returns The number of bytes read, or error.
LLVM_SUPPORT_ABI Expected<size_t> 
readNativeFileSlice(file_t FileHandle,
                    MutableArrayRef<char> Buf,
                    uint64_t Offset);
...
LLVM_SUPPORT_ABI std::error_code lockFile(int FD);
...
LLVM_SUPPORT_ABI std::error_code getUniqueID(const Twine Path,
                                             UniqueID &Result);
...
LLVM_SUPPORT_ABI ErrorOr<space_info> disk_space(const Twine &Path);
...
public:
  ...
  LLVM_SUPPORT_ABI size_t size() const;
  LLVM_SUPPORT_ABI char *data() const;

  /// Get a const view of the data. Modifying this memory has undefined
  /// behavior.
  LLVM_SUPPORT_ABI const char *const_data() const;

  /// \returns The minimum alignment offset must be.
  LLVM_SUPPORT_ABI static int alignment();
};

/// Return the path to the main executable, given the value of argv[0] from
/// program startup and the address of main itself. In extremis, this function
/// may fail and return an empty path.
LLVM_SUPPORT_ABI std::string getMainExecutable(const char *argv0,
                                               void *MainExecAddr);
...
public:
  ...
  LLVM_SUPPORT_ABI ErrorOr<basic_file_status> status() const;
  ...
  LLVM_SUPPORT_ABI bool operator< (const directory_entry& RHS) const;
  LLVM_SUPPORT_ABI bool operator<=(const directory_entry& RHS) const;
  LLVM_SUPPORT_ABI bool operator> (const directory_entry& RHS) const;
  LLVM_SUPPORT_ABI bool operator>=(const directory_entry& RHS) const;
};

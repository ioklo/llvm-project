...
namespace sys {
  ...
  LLVM_SUPPORT_ABI bool RemoveFileOnSignal(StringRef Filename,
                                           std::string *ErrMsg = nullptr);

  /// This function removes a file from the list of files to be removed on
  /// signal delivery.
  LLVM_SUPPORT_ABI void DontRemoveFileOnSignal(StringRef Filename);
  ...
  PrintStackTraceOnErrorSignal(StringRef Argv0,
  ...
  LLVM_SUPPORT_ABI void RunSignalHandlers();
  ...
  LLVM_SUPPORT_ABI void AddSignalHandler(SignalHandlerCallback FnPtr,
                                         void *Cookie);
  ...
  LLVM_SUPPORT_ABI void SetInfoSignalFunction(void (*Handler)());
  ...
  /// different thread on some platforms.
  LLVM_SUPPORT_ABI void SetOneShotPipeSignalFunction(void (*Handler)());

  /// On Unix systems and Windows, this function exits with an "IO error" exit
  /// code.
  LLVM_SUPPORT_ABI void DefaultOneShotPipeSignalHandler();

#ifdef _WIN32
  /// Windows does not support signals and this handler must be called manually.
  LLVM_SUPPORT_ABI void CallOneShotPipeSignalHandler();
#endif
  ...
  LLVM_SUPPORT_ABI void CleanupOnSignal(uintptr_t Context);

  LLVM_SUPPORT_ABI void unregisterHandlers();
} // End sys namespace
} // End llvm namespace

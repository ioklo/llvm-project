...
private:
  ...
  LLVM_SUPPORT_ABI static KnownBits flipSignBit(const KnownBits &Val);
...
public:
  ...
  /// tracking.
  LLVM_SUPPORT_ABI KnownBits sextInReg(unsigned SrcBitWidth) const;
  ...
  LLVM_SUPPORT_ABI KnownBits makeGE(const APInt &Val) const;
  ...
  LLVM_SUPPORT_ABI static KnownBits computeForAddCarry(
      const KnownBits &LHS, const KnownBits &RHS, const KnownBits &Carry);

  /// Compute known bits resulting from adding LHS and RHS.
  LLVM_SUPPORT_ABI static KnownBits computeForAddSub(bool Add, bool NSW,
                                                     bool NUW,
                                    const KnownBits &LHS, const KnownBits &RHS);

  /// Compute known bits results from subtracting RHS from LHS with 1-bit
  /// Borrow.
  LLVM_SUPPORT_ABI static KnownBits
  computeForSubBorrow(const KnownBits &LHS, KnownBits RHS,
  ...
  LLVM_SUPPORT_ABI static KnownBits sadd_sat(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute knownbits resulting from llvm.uadd.sat(LHS, RHS)
  LLVM_SUPPORT_ABI static KnownBits uadd_sat(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute knownbits resulting from llvm.ssub.sat(LHS, RHS)
  LLVM_SUPPORT_ABI static KnownBits ssub_sat(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute knownbits resulting from llvm.usub.sat(LHS, RHS)
  LLVM_SUPPORT_ABI static KnownBits usub_sat(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute knownbits resulting from APIntOps::avgFloorS
  LLVM_SUPPORT_ABI static KnownBits avgFloorS(const KnownBits &LHS,
                                              const KnownBits &RHS);

  /// Compute knownbits resulting from APIntOps::avgFloorU
  LLVM_SUPPORT_ABI static KnownBits avgFloorU(const KnownBits &LHS,
                                              const KnownBits &RHS);

  /// Compute knownbits resulting from APIntOps::avgCeilS
  LLVM_SUPPORT_ABI static KnownBits avgCeilS(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute knownbits resulting from APIntOps::avgCeilU
  LLVM_SUPPORT_ABI static KnownBits avgCeilU(const KnownBits &LHS,
                                             const KnownBits &RHS);

  /// Compute known bits resulting from multiplying LHS and RHS.
  LLVM_SUPPORT_ABI static KnownBits mul(const KnownBits &LHS,
                                        const KnownBits &RHS,
                       bool NoUndefSelfMultiply = false);

  /// Compute known bits from sign-extended multiply-hi.
  LLVM_SUPPORT_ABI static KnownBits mulhs(const KnownBits &LHS,
                                          const KnownBits &RHS);

  /// Compute known bits from zero-extended multiply-hi.
  LLVM_SUPPORT_ABI static KnownBits mulhu(const KnownBits &LHS,
                                          const KnownBits &RHS);

  /// Compute known bits for sdiv(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits
  sdiv(const KnownBits &LHS, const KnownBits &RHS,
                        bool Exact = false);

  /// Compute known bits for udiv(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits
  udiv(const KnownBits &LHS, const KnownBits &RHS,
                        bool Exact = false);

  /// Compute known bits for urem(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits urem(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for srem(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits srem(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for umax(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits umax(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for umin(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits umin(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for smax(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits smax(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for smin(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits smin(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for abdu(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits abdu(const KnownBits &LHS,
                                         const KnownBits &RHS);

  /// Compute known bits for abds(LHS, RHS).
  LLVM_SUPPORT_ABI static KnownBits abds(KnownBits LHS, KnownBits RHS);

  /// Compute known bits for shl(LHS, RHS).
  /// NOTE: RHS (shift amount) bitwidth doesn't need to be the same as LHS.
  LLVM_SUPPORT_ABI static KnownBits shl(const KnownBits &LHS,
                                        const KnownBits &RHS,
  ...
  /// NOTE: RHS (shift amount) bitwidth doesn't need to be the same as LHS.
  LLVM_SUPPORT_ABI static KnownBits lshr(const KnownBits &LHS,
                                         const KnownBits &RHS,
                                         bool ShAmtNonZero = false, bool Exact = false);

  /// Compute known bits for ashr(LHS, RHS).
  /// NOTE: RHS (shift amount) bitwidth doesn't need to be the same as LHS.
  LLVM_SUPPORT_ABI static KnownBits ashr(const KnownBits &LHS,
                                         const KnownBits &RHS,
                                         bool ShAmtNonZero = false, bool Exact = false);

  /// Determine if these known bits always give the same ICMP_EQ result.
  LLVM_SUPPORT_ABI static std::optional<bool> eq(const KnownBits &LHS,
                                                 const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_NE result.
  LLVM_SUPPORT_ABI static std::optional<bool> ne(const KnownBits &LHS,
                                                 const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_UGT result.
  LLVM_SUPPORT_ABI static std::optional<bool> ugt(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_UGE result.
  LLVM_SUPPORT_ABI static std::optional<bool> uge(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_ULT result.
  LLVM_SUPPORT_ABI static std::optional<bool> ult(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_ULE result.
  LLVM_SUPPORT_ABI static std::optional<bool> ule(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_SGT result.
  LLVM_SUPPORT_ABI static std::optional<bool> sgt(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_SGE result.
  LLVM_SUPPORT_ABI static std::optional<bool> sge(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_SLT result.
  LLVM_SUPPORT_ABI static std::optional<bool> slt(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Determine if these known bits always give the same ICMP_SLE result.
  LLVM_SUPPORT_ABI static std::optional<bool> sle(const KnownBits &LHS,
                                                  const KnownBits &RHS);

  /// Update known bits based on ANDing with RHS.
  LLVM_SUPPORT_ABI KnownBits &operator&=(const KnownBits &RHS);
  ...
  LLVM_SUPPORT_ABI KnownBits abs(bool IntMinIsPoison = false) const;
  ...
  LLVM_SUPPORT_ABI KnownBits blsi() const;
  ...
  LLVM_SUPPORT_ABI KnownBits blsmsk() const;
...
private:
  ...
  LLVM_SUPPORT_ABI static KnownBits remGetLowBits(const KnownBits &LHS,
                                                  const KnownBits &RHS);
};

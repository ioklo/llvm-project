...
public:
  ...
  ///                       brace expansion
  LLVM_SUPPORT_ABI static Expected<GlobPattern>
  create(StringRef Pat, std::optional<size_t> MaxSubPatterns = {});
  /// \returns \p true if \p S matches this glob pattern
  LLVM_SUPPORT_ABI bool match(StringRef S) const;

  // Returns true for glob pattern "*". Can be used to avoid expensive
  // preparation/acquisition of the input for match().
  LLVM_SUPPORT_ABI bool isTrivialMatchAll() const {
    if (!Prefix.empty())
      return false;
    if (SubGlobs.size() != 1)
      return false;
...
private:
  ...
  struct SubGlobPattern {
    /// \param Pat the pattern to match against
    LLVM_SUPPORT_ABI static Expected<SubGlobPattern> create(StringRef Pat);
    /// \returns \p true if \p S matches this glob pattern
    LLVM_SUPPORT_ABI bool match(StringRef S) const;

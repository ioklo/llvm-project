...
public:
  LLParser(StringRef F, SourceMgr &SM, SMDiagnostic &Err, Module *M,
      ...
      : Context(Context), OPLex(F, SM, Err, Context), Lex(F, SM, Err, Context),
        M(M), Index(Index), Slots(Slots), BlockAddressPFS(nullptr) {}
  LLVM_ASMPARSER_ABI bool Run(
      bool UpgradeDebugInfo,
      DataLayoutCallbackTy DataLayoutCallback = [](StringRef, StringRef) {
        return std::nullopt;
      });

  LLVM_ASMPARSER_ABI bool
  parseStandaloneConstantValue(Constant *&C, const SlotMapping *Slots);

  LLVM_ASMPARSER_ABI bool parseTypeAtBeginning(Type *&Ty, unsigned &Read,
                                               const SlotMapping *Slots);

  LLVM_ASMPARSER_ABI bool
  parseDIExpressionBodyAtBeginning(MDNode *&Result, unsigned &Read,
                                   const SlotMapping *Slots);

  LLVM_ASMPARSER_ABI LLVMContext &getContext() { return Context; }

private:
  bool error(LocTy L, const Twine &Msg) { return Lex.ParseError(L, Msg); }
  bool tokError(const Twine &Msg) { return error(Lex.getLoc(), Msg); }

  bool checkValueID(LocTy L, StringRef Kind, StringRef Prefix, unsigned NextID,
                    unsigned ID);
  ...
  // Constant Parsing.
  bool parseValID(ValID &ID, PerFunctionState *PFS, Type *ExpectedTy = nullptr);

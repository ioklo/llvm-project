...
LLVM_BITREADER_ABI std::error_code
errorToErrorCodeAndEmitErrors(LLVMContext &Ctx, Error Err);
...
struct ParserCallbacks {
  ...
  class BitcodeModule {
    ...
    // Calls the ctor.
    friend LLVM_BITREADER_ABI Expected<BitcodeFileContents>
  ...
  LLVM_BITREADER_ABI Expected<BitcodeFileContents>
  getBitcodeFileContents(MemoryBufferRef Buffer);

  /// Returns a list of modules in the specified bitcode buffer.
  LLVM_BITREADER_ABI Expected<std::vector<BitcodeModule>>
  ...
  /// returns "".
  LLVM_BITREADER_ABI Expected<std::string>
  getBitcodeTargetTriple(MemoryBufferRef Buffer);

  /// Return true if \p Buffer contains a bitcode file with ObjC code (category
  /// or class) in it.
  LLVM_BITREADER_ABI Expected<bool>
  ...
  LLVM_BITREADER_ABI Expected<BitcodeLTOInfo>
  getBitcodeLTOInfo(MemoryBufferRef Buffer);

  /// Parse the specified bitcode buffer, returning the module summary index.
  LLVM_BITREADER_ABI Expected<std::unique_ptr<ModuleSummaryIndex>>
  getModuleSummaryIndex(MemoryBufferRef Buffer);

  /// Parse the specified bitcode buffer and merge the index into CombinedIndex.
  LLVM_BITREADER_ABI Error readModuleSummaryIndex(
  ...
  LLVM_BITREADER_ABI APInt readWideAPInt(ArrayRef<uint64_t> Vals,
                                         unsigned TypeBits);

  LLVM_BITREADER_ABI const std::error_category &BitcodeErrorCategory();

...
LLVM_TARGETPARSER_ABI const std::vector<FMVInfo> &getFMVInfo();
...
struct ExtensionSet {
  ...
  LLVM_TARGETPARSER_ABI void enable(ArchExtKind E);
  ...
  LLVM_TARGETPARSER_ABI void addArchDefaults(const ArchInfo &Arch);
  ...
  // between them.
  LLVM_TARGETPARSER_ABI bool parseModifier(StringRef Modifier,
...
LLVM_TARGETPARSER_ABI const ExtensionInfo &getExtensionByID(ArchExtKind(ExtID));

LLVM_TARGETPARSER_ABI bool
getExtensionFeatures(const AArch64::ExtensionBitset &Extensions,
                     std::vector<StringRef> &Features);

LLVM_TARGETPARSER_ABI StringRef getArchExtFeature(StringRef ArchExt);
LLVM_TARGETPARSER_ABI StringRef resolveCPUAlias(StringRef CPU);

// Information by Name
LLVM_TARGETPARSER_ABI const ArchInfo *getArchForCpu(StringRef CPU);

// Parser
LLVM_TARGETPARSER_ABI const ArchInfo *parseArch(StringRef Arch);

// Return the extension which has the given -target-feature name.
LLVM_TARGETPARSER_ABI std::optional<ExtensionInfo>
...
LLVM_TARGETPARSER_ABI std::optional<FMVInfo>
parseFMVExtension(StringRef Extension);

// Given the name of a CPU or alias, return the correponding CpuInfo.
LLVM_TARGETPARSER_ABI std::optional<CpuInfo> parseCpu(StringRef Name);
// Used by target parser tests
LLVM_TARGETPARSER_ABI void
fillValidCPUArchList(SmallVectorImpl<StringRef> &Values);

LLVM_TARGETPARSER_ABI bool isX18ReservedByDefault(const Triple &TT);
...
LLVM_TARGETPARSER_ABI uint64_t getFMVPriority(ArrayRef<StringRef> Features);
...
LLVM_TARGETPARSER_ABI uint64_t getCpuSupportsMask(ArrayRef<StringRef> Features);

LLVM_TARGETPARSER_ABI void PrintSupportedExtensions();

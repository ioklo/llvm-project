...
namespace llvm {
namespace PPC {
LLVM_TARGETPARSER_ABI bool isValidCPU(StringRef CPU);
...
getNormalizedPPCTargetCPU(const Triple &T, StringRef CPUName = "");

// Get the tune CPU name.
LLVM_TARGETPARSER_ABI StringRef getNormalizedPPCTuneCPU(const Triple &T,
                                                        StringRef CPUName = "");

// For PPC, there are some cpu names for same CPU, like pwr10 and power10,
// normalize them.
LLVM_TARGETPARSER_ABI StringRef normalizeCPUName(StringRef CPUName);
} // namespace PPC
} // namespace llvm

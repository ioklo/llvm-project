...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI Error extract(
      const DWARFDataExtractor &Data, uint64_t *OffsetPtr, uint16_t CUVersion,
      uint8_t CUAddrSize, std::function<void(Error)> WarnCallback);

  /// Extract a DWARFv5 address table.
  LLVM_DEBUGINFODWARF_ABI Error
  extractV5(const DWARFDataExtractor &Data, uint64_t *OffsetPtr,
            uint8_t CUAddrSize, std::function<void(Error)> WarnCallback);
  ...
  LLVM_DEBUGINFODWARF_ABI Error
  extractPreStandard(const DWARFDataExtractor &Data, uint64_t *OffsetPtr,
  ...
  LLVM_DEBUGINFODWARF_ABI Expected<uint64_t> getAddrEntry(uint32_t Index) const;

  /// Return the full length of this table, including the length field.
  /// Return std::nullopt if the length cannot be identified reliably.
  LLVM_DEBUGINFODWARF_ABI std::optional<uint64_t> getFullLength() const;

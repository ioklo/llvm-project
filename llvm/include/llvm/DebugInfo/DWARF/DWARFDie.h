...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI bool isSubprogramDIE() const;

  /// Returns true if DIE represents a subprogram or an inlined subroutine.
  LLVM_DEBUGINFODWARF_ABI bool isSubroutineDIE() const;

  /// Get the parent of this DIE object.
  ///
  /// \returns a valid DWARFDie instance if this object has a parent or an
  /// invalid DWARFDie instance if it doesn't.
  LLVM_DEBUGINFODWARF_ABI DWARFDie getParent() const;

  /// Get the sibling of this DIE object.
  ///
  /// \returns a valid DWARFDie instance if this object has a sibling or an
  /// invalid DWARFDie instance if it doesn't.
  LLVM_DEBUGINFODWARF_ABI DWARFDie getSibling() const;

  /// Get the previous sibling of this DIE object.
  ///
  /// \returns a valid DWARFDie instance if this object has a sibling or an
  /// invalid DWARFDie instance if it doesn't.
  LLVM_DEBUGINFODWARF_ABI DWARFDie getPreviousSibling() const;

  /// Get the first child of this DIE object.
  ///
  /// \returns a valid DWARFDie instance if this object has children or an
  /// invalid DWARFDie instance if it doesn't.
  LLVM_DEBUGINFODWARF_ABI DWARFDie getFirstChild() const;

  /// Get the last child of this DIE object.
  ///
  /// \returns a valid null DWARFDie instance if this object has children or an
  /// invalid DWARFDie instance if it doesn't.
  LLVM_DEBUGINFODWARF_ABI DWARFDie getLastChild() const;
  ...
  dump(raw_ostream &OS, unsigned indent = 0,
       DIDumpOptions DumpOpts = DIDumpOptions()) const;

  /// Convenience zero-argument overload for debugging.
  LLVM_DEBUGINFODWARF_ABI LLVM_DUMP_METHOD void dump() const;
  ...
  LLVM_DEBUGINFODWARF_ABI std::optional<DWARFFormValue>
  find(dwarf::Attribute Attr) const;
  ...
  LLVM_DEBUGINFODWARF_ABI DWARFDie
  getAttributeValueAsReferencedDie(dwarf::Attribute Attr) const;
  LLVM_DEBUGINFODWARF_ABI DWARFDie
  getAttributeValueAsReferencedDie(const DWARFFormValue &V) const;

  LLVM_DEBUGINFODWARF_ABI DWARFDie resolveTypeUnitReference() const;

  LLVM_DEBUGINFODWARF_ABI DWARFDie
  resolveReferencedType(dwarf::Attribute Attr) const;
  LLVM_DEBUGINFODWARF_ABI DWARFDie
  ...
  LLVM_DEBUGINFODWARF_ABI std::optional<uint64_t> getLocBaseAttribute() const;
  ...
  LLVM_DEBUGINFODWARF_ABI bool getLowAndHighPC(uint64_t &LowPC,
                                               uint64_t &HighPC,
  ...
  LLVM_DEBUGINFODWARF_ABI Expected<DWARFAddressRangesVector>
  getAddressRanges() const;

  LLVM_DEBUGINFODWARF_ABI bool
  addressRangeContainsAddress(const uint64_t Address) const;

  LLVM_DEBUGINFODWARF_ABI std::optional<uint64_t> getLanguage() const;

  LLVM_DEBUGINFODWARF_ABI Expected<DWARFLocationExpressionsVector>
  ...
  LLVM_DEBUGINFODWARF_ABI const char *getSubroutineName(DINameKind Kind) const;
  ...
  /// Returns null if no name is found.
  LLVM_DEBUGINFODWARF_ABI const char *getName(DINameKind Kind) const;
  LLVM_DEBUGINFODWARF_ABI void
  getFullName(raw_string_ostream &,
  ...
  /// is found.
  LLVM_DEBUGINFODWARF_ABI const char *getShortName() const;

  /// Return the DIE linkage name resolving DW_AT_specification or
  /// DW_AT_abstract_origin references if necessary. Returns null if no name
  /// is found.
  LLVM_DEBUGINFODWARF_ABI const char *getLinkageName() const;
  ...
  LLVM_DEBUGINFODWARF_ABI uint64_t getDeclLine() const;
  LLVM_DEBUGINFODWARF_ABI std::string
  ...
  LLVM_DEBUGINFODWARF_ABI void
  getCallerFrame(uint32_t &CallFile, uint32_t &CallLine, uint32_t &CallColumn,
  ...
  LLVM_DEBUGINFODWARF_ABI iterator_range<attribute_iterator> attributes() const;
  ...
  LLVM_DEBUGINFODWARF_ABI iterator begin() const;
  LLVM_DEBUGINFODWARF_ABI iterator end() const;

  LLVM_DEBUGINFODWARF_ABI std::reverse_iterator<iterator> rbegin() const;
  LLVM_DEBUGINFODWARF_ABI std::reverse_iterator<iterator> rend() const;

  LLVM_DEBUGINFODWARF_ABI iterator_range<iterator> children() const;
};
...
class DWARFDie::attribute_iterator
  ...
  LLVM_DEBUGINFODWARF_ABI void
  updateForIndex(const DWARFAbbreviationDeclaration &AbbrDecl, uint32_t I);

public:
  attribute_iterator() = delete;
  LLVM_DEBUGINFODWARF_ABI explicit attribute_iterator(DWARFDie D, bool End);

  LLVM_DEBUGINFODWARF_ABI attribute_iterator &operator++();
  LLVM_DEBUGINFODWARF_ABI attribute_iterator &operator--();
...
LLVM_DEBUGINFODWARF_ABI void dumpTypeQualifiedName(const DWARFDie &DIE,
                                                   raw_ostream &OS);
LLVM_DEBUGINFODWARF_ABI void
dumpTypeUnqualifiedName(const DWARFDie &DIE, raw_ostream &OS,

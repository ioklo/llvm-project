...
public:
  ...
  /// the unwind opcodes.
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation createUnspecified();
  ...
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation createUndefined();
  /// Create a location where the value is known to be in the register itself.
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation createSame();
  ...
  /// information.
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation
  ...
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation
  createIsDWARFExpression(DWARFExpression Expr);
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation
  createAtDWARFExpression(DWARFExpression Expr);
  LLVM_DEBUGINFODWARF_ABI static UnwindLocation createIsConstant(int32_t Value);
  ...
  LLVM_DEBUGINFODWARF_ABI bool operator==(const UnwindLocation &RHS) const;
};
...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI void dump(raw_ostream &OS, DIDumpOptions DumpOpts,
...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI static Expected<UnwindTable> create(const CIE *Cie);
  ...
  /// \returns An error if the DWARF Call Frame Information opcodes have state
  /// machine errors, or a valid UnwindTable otherwise.
  LLVM_DEBUGINFODWARF_ABI static Expected<UnwindTable> create(const FDE *Fde);
...
public:
  ...
  struct Instruction {
    ...
    getOperandAsUnsigned(const CFIProgram &CFIP, uint32_t OperandIdx) const;

    LLVM_DEBUGINFODWARF_ABI Expected<int64_t>
    getOperandAsSigned(const CFIProgram &CFIP, uint32_t OperandIdx) const;
  };
  ...
  LLVM_DEBUGINFODWARF_ABI Error parse(DWARFDataExtractor Data, uint64_t *Offset,
                                      uint64_t EndOffset);

  LLVM_DEBUGINFODWARF_ABI void
  dump(raw_ostream &OS, DIDumpOptions DumpOpts, unsigned IndentLevel,
       std::optional<uint64_t> InitialLocation) const;

  void addInstruction(const Instruction &I) { Instructions.push_back(I); }

  /// Get a DWARF CFI call frame string for the given DW_CFA opcode.
  LLVM_DEBUGINFODWARF_ABI StringRef callFrameString(unsigned Opcode) const;
...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI void dump(raw_ostream &OS,
                                    DIDumpOptions DumpOpts) const override;
...
public:
  ...
  LLVM_DEBUGINFODWARF_ABI DWARFDebugFrame(Triple::ArchType Arch,
                                          bool IsEH = false,
                                          uint64_t EHFrameAddress = 0);
  LLVM_DEBUGINFODWARF_ABI ~DWARFDebugFrame();
  ...
  LLVM_DEBUGINFODWARF_ABI Error parse(DWARFDataExtractor Data);

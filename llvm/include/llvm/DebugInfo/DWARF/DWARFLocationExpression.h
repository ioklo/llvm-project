...
struct DWARFLocationExpression {
  /// The address range in which this expression is valid. std::nullopt denotes
  /// a default entry which is valid in addresses not covered by other location
...
LLVM_DEBUGINFODWARF_ABI raw_ostream &
operator<<(raw_ostream &OS, const DWARFLocationExpression &Loc);

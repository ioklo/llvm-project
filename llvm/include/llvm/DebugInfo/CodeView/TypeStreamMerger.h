...
LLVM_DEBUGINFOCODEVIEW_ABI Error mergeTypeRecords(
    MergingTypeTableBuilder &Dest, SmallVectorImpl<TypeIndex> &SourceToDest,
...
/// \returns Error::success() if the operation succeeded, otherwise an
/// appropriate error code.
LLVM_DEBUGINFOCODEVIEW_ABI Error mergeIdRecords(
    MergingTypeTableBuilder &Dest, ArrayRef<TypeIndex> Types,
    SmallVectorImpl<TypeIndex> &SourceToDest, const CVTypeArray &Ids);
...
/// \returns Error::success() if the operation succeeded, otherwise an
/// appropriate error code.
LLVM_DEBUGINFOCODEVIEW_ABI Error mergeTypeAndIdRecords(
...
LLVM_DEBUGINFOCODEVIEW_ABI Error mergeTypeRecords(
    GlobalTypeTableBuilder &Dest, SmallVectorImpl<TypeIndex> &SourceToDest,
    const CVTypeArray &Types, ArrayRef<GloballyHashedType> Hashes,
    std::optional<PCHMergerInfo> &PCHInfo);

LLVM_DEBUGINFOCODEVIEW_ABI Error
mergeIdRecords(GlobalTypeTableBuilder &Dest, ArrayRef<TypeIndex> Types,
               SmallVectorImpl<TypeIndex> &SourceToDest, const CVTypeArray &Ids,

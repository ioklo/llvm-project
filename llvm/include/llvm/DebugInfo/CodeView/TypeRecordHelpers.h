...
/// option.
LLVM_DEBUGINFOCODEVIEW_ABI bool isUdtForwardRef(CVType CVT);

/// Given a CVType which is assumed to be an LF_MODIFIER, return the
/// TypeIndex of the type that the LF_MODIFIER modifies.
LLVM_DEBUGINFOCODEVIEW_ABI TypeIndex getModifiedType(const CVType &CVT);
...
LLVM_DEBUGINFOCODEVIEW_ABI uint64_t getSizeInBytesForTypeIndex(TypeIndex TI);
...
LLVM_DEBUGINFOCODEVIEW_ABI uint64_t getSizeInBytesForTypeRecord(CVType CVT);

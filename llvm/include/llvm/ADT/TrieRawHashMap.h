...
protected:
  ...
  LLVM_SUPPORT_ABI PointerBase find(ArrayRef<uint8_t> Hash) const;

  /// Insert and return the stored content.
  LLVM_SUPPORT_ABI PointerBase
  ...
  LLVM_SUPPORT_ABI ThreadSafeTrieRawHashMapBase(
  ...
  destroyImpl(function_ref<void(void *ValueMem)> Destructor);
  ...
  // thread-safe.
  LLVM_SUPPORT_ABI PointerBase getRoot() const;
  LLVM_SUPPORT_ABI unsigned getStartBit(PointerBase P) const;
  LLVM_SUPPORT_ABI unsigned getNumBits(PointerBase P) const;
  LLVM_SUPPORT_ABI unsigned getNumSlotUsed(PointerBase P) const;
  LLVM_SUPPORT_ABI std::string getTriePrefixAsString(PointerBase P) const;
  LLVM_SUPPORT_ABI unsigned getNumTries() const;
  // Visit next trie in the allocation chain.
  LLVM_SUPPORT_ABI PointerBase getNextTrie(PointerBase P) const;

...
public:
  LLVM_SUPPORT_ABI DeltaTree();

  // Note: Currently we only support copying when the RHS is empty.
  LLVM_SUPPORT_ABI DeltaTree(const DeltaTree &RHS);
  ...
  LLVM_SUPPORT_ABI int getDeltaAt(unsigned FileIndex) const;

  /// AddDelta - When a change is made that shifts around the text buffer,
  /// this method is used to record that info.  It inserts a delta of 'Delta'
  /// into the current DeltaTree at offset FileIndex.
  LLVM_SUPPORT_ABI void AddDelta(unsigned FileIndex, int Delta);
};

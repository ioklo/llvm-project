...
protected:
  ...
  LLVM_SUPPORT_ABI StringMapImpl(unsigned InitSize, unsigned ItemSize);
  ~StringMapImpl() { free(TheTable); }
  LLVM_SUPPORT_ABI unsigned RehashTable(unsigned BucketNo = 0);
  ...
  LLVM_SUPPORT_ABI unsigned LookupBucketFor(StringRef Key,
  ...
  /// Overload that explicitly takes precomputed hash(Key).
  LLVM_SUPPORT_ABI int FindKey(StringRef Key, uint32_t FullHashValue) const;

  /// RemoveKey - Remove the specified StringMapEntry from the table, but do not
  /// delete it.  This aborts if the value isn't in the table.
  LLVM_SUPPORT_ABI void RemoveKey(StringMapEntryBase *V);

  /// RemoveKey - Remove the StringMapEntry for the specified key from the
  /// table, returning it.  If the key is not in the table, this returns null.
  LLVM_SUPPORT_ABI StringMapEntryBase *RemoveKey(StringRef Key);

  /// Allocate the table with the specified number of buckets and otherwise
  /// setup the map as empty.
  LLVM_SUPPORT_ABI void init(unsigned Size);
...
public:
  ...
  /// and may change.
  LLVM_SUPPORT_ABI static uint32_t hash(StringRef Key);
